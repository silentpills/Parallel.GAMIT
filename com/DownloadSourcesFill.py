...
def main():
    ...
    if args.skip_stations_with_source:
        ...
        for stn in stnlist:
            rs = cnn.query(
                """SELECT count(*) as c FROM sources_stations WHERE
        ...
        if not len(stnlist):
            ...
            print(
                """ >> Stations with source removed.
                  Stations without source: %i"""
                % len(stnlist)
            )
    ...
    if not args.data_source:
        ...
        try:
            ...
            rs = cnn.query(
                """SELECT * FROM sources_servers
                           WHERE server_id = %i  """
                % int(args.data_source)
            )
        except ValueError:
            # FQDN
            rs = cnn.query(
                "SELECT * FROM sources_servers WHERE fqdn = '%s'" % args.data_source
            )
    ...
    if rs is not None:
        for svr in rs.dictresult():
            # connect to ftp
            tqdm.write(
                " >> Working with source %s://%s (server id: %i)"
            ...
            tqdm.write(
                " >> Searching source: %i potential loops..."
                % (len(stnlist) * len(drange))
            )

            if "${station}" in os.path.dirname(
                svr["path"]
            ...
            tqdm.write(
                " >> Done searching, found %i matches for %s://%s\n"
            ...
            for match in match_list:
                ...
                srcs = cnn.query(
                    """SELECT sources_servers.fqdn,
                                 ...
                                 AND "StationCode" = \'%s\'
                                 ORDER BY try_order"""
                    % (match["NetworkCode"], match["StationCode"])
                ).dictresult()

                if len(srcs):
                    if svr["server_id"] not in [s["server_id"] for s in srcs]:
                        tqdm.write(
                            """ >> Station %s does not have this source
                                   (but these other):"""
                            % stationID(match)
                        )
                        for src in srcs:
                            tqdm.write(
                                " -- try order %i source: %s "
                ...
                if ask_add:
                    if args.force_yes:
                        ...
                        if query_yes_no(
                            """    Would like like to add %s
                                        as a source? (source filename %s)"""
                            % (svr["fqdn"], match["filename"]),
                            None,
                        ):
                            add = True
                        else:
...
def search_by_date(stnlist, drange, svr, client):
    ...
    for date in (pyDate.Date(mjd=mdj) for mdj in reversed(drange)):
        ...
        for stn in stnlist:
            ...
            if stn not in match_list:
                filename = path_replace_tags(
                    os.path.basename(svr["path"]),
                    date,
                    stn["NetworkCode"],
                    stn["StationCode"],
                    stn["marker"],
