...
def main():
    parser = argparse.ArgumentParser(
        description="Program to alter the default ETM parameters for each station. "
...
def insert_modify_param(parser, cnn, stnlist, args):
    ...
    if len(args.function_type) < 2:
        ...
        parser.error(
            "function type should be one of the following: polynomial (p), jump (j), periodic (q), "
    ...
    tpar = Bunch()
    tpar.NetworkCode = tpar.StationCode = tpar.soln = tpar.object = tpar.terms = (
        tpar.frequencies
    ) = None
            ...
            if tpar.jump_type >= 1:
                ...
                if not tpar.relaxation:
                    if tpar.action == "-":
                        ...
                        parser.error(
                            "jump type == 1 but no relaxation parameter, please specify relaxation"
                        )
...
def apply_change(cnn, station, tpar, soln):
    ...
    ppar = {
        ...
        if v not in (None, [])
        and k not in ("action", "relaxation", "jump_type", "terms", "frequencies")
    }
