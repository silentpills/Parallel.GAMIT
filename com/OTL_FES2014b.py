...
def create_files(stnlist):
    ...
    if stnlist:
        ...
        rs = cnn.query(
            'SELECT * FROM stations WHERE "NetworkCode" NOT LIKE \'?%%\' AND "Harpos_coeff_otl" LIKE '
    ...
    for stn in stations:
        stnlist += [
            ...
            % (
                stn["NetworkCode"] + "_" + stn["StationCode"],
                float(stn["auto_x"]),
                float(stn["auto_y"]),
                float(stn["auto_z"]),
            )
        ...
        if len(stnlist) == 99:
            ...
            file_write("otl_%i.list" % index, body)
...
def process_blq(filename):
    ...
    try:
        ...
        for stn in blq_otl:
            print(
                " >> Updating OTL for %s.%s" % (stn["NetworkCode"], stn["StationCode"])
            )
            cnn.update(
                "stations",
                {"Harpos_coeff_otl": stn["otl"]},
...
def load_harpos(header, otl):
    ...
    OTL = ("".join(header) + "".join(otl)).replace(
        NetStn[0], StationCode + "    "
    ) + "HARPOS Format version of 2002.12.12"

    print(" >> updating %s.%s" % (NetworkCode, StationCode))
    cnn.query(
        "UPDATE stations SET \"Harpos_coeff_otl\" = '%s' WHERE \"NetworkCode\" = '%s' AND "
