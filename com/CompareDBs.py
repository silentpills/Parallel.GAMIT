...
def main():
    ...
    for stn in stns:
        sp = pyPPP.PPPSpatialCheck(
            [float(stn["lat"])], [float(stn["lon"])], [float(stn["height"])]
        )

        found, match, stn_li = sp.verify_spatial_coherence(cnn2, stn["StationCode"])
        ...
        if found:
            ...
            if len(match) > 0:
                ...
                shutil.move(
                    "%s.zip" % stationID(stn), "export/maybe/%s.zip" % stationID(stn)
                )
            else:
                ...
                # move the file into the folder
                shutil.move(
                    "%s.zip" % stationID(stn), "export/sure/%s.zip" % stationID(stn)
                )
