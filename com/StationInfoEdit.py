...
# app
from geode import dbConnection, pyDate, pyOptions
from geode.metadata.station_info import StationInfo, StationInfoRecord
from geode.Utils import add_version_argument, process_date
...
class Menu(object):
    ...
    def validate(self, edit_field):
        ...
        if fname in ("DateStart", "DateEnd"):
            ...
            rs = self.cnn.query(
                "SELECT * FROM antennas WHERE \"AntennaCode\" = '%s'"
                % (edit_field.upper())
            )
            ...
            rs = self.cnn.query(
                "SELECT * FROM receivers WHERE \"ReceiverCode\" = '%s'"
                % (edit_field.upper())
            )
            ...
            if (
                not (
                    re.findall(r"^\d+[.]?\d*[DEde+-]?\d*$", edit_field)
                    or edit_field in ("-", "--", "---", "----", "-----")
                )
            ) or len(edit_field) > 5:
                self.ShowError(
                    "Receiver Firmware format must be one of these: d.d-d, d.d+d, dEd, d.d, or five "
                    "dashes (-----)"
                )
                return False
            elif edit_field in ("-", "--", "---", "----", "-----"):
...
def save_changes(menu):
    ...
    if menu.edited_fields.keys():
        record = {"NetworkCode": stn["NetworkCode"], "StationCode": stn["StationCode"]}
        ...
        record = StationInfoRecord(
            stn["NetworkCode"], stn["StationCode"], _record=record
        )
...
def selection_main_menu(menu):
    ...
    if menu.position < StnInfo.record_count:
        ...
        record["AntennaHeight"] = (
            "0.000" if stninfo is None else stninfo["AntennaHeight"]
        )
        ...
        record["ReceiverFirmware"] = (
            "" if stninfo is None else stninfo["ReceiverFirmware"]
        )
        ...
        edit_menu = Menu(
            ...
            new_record,
            screen,
            "New station information record (ESC to cancel)",
            "edit",
        )
    ...
    menu.items += [
        {
            "field": "Insert new station information record",
            "function": selection_main_menu,
        }
    ]
...
def get_fields(position):
    ...
    for field, value in record.items():
        if field not in ["NetworkCode", "StationCode"]:
            out.append(
                {"field": field, "value": (value if type(value) is str else str(value))}
            )
...
class MyApp(object):
    def __init__(self, stdscreen):
        ...
        main_menu = Menu(
            ...
            main_menu_items,
            self.screen,
            "Station information new/edit/delete - %s.%s"
...
def main():
    ...
    parser.add_argument(
        "stn", type=str, nargs=1, help="Station name given in net.stnm format."
    )
    ...
    if "." in stn:
        ...
        rs = cnn.query(
            'SELECT * FROM stations WHERE "StationCode" = \'%s\' ORDER BY "NetworkCode", "StationCode"'
            % stn
        )
