...
from geode.etm.core.etm_engine import EtmEncoder, enum_dict_factory
...
def main():
    parser = argparse.ArgumentParser(
        description="Create, extract, and combine ETM functions"
    )
    ...
    if os.path.exists(args.station):
        ...
        config = EtmConfig(
            station["network_code"], station["station_code"], json_file=args.station
        )
    else:
    ...
    config.metadata.station_information = [
        item.to_json() for item in config.metadata.station_information
    ]
    output_json = {
        "station_meta": asdict(config.metadata, dict_factory=enum_dict_factory),
        "functions": load_json(args.output)["functions"]
        ...
        for input_opt in args.input_json:
            output_json["functions"].extend(
                [
                    ...
                    for funct in get_prefit_models(config, input_opt)
                ]
            )

    if args.model:
        for model in args.model:
            output_json["functions"].append(
                asdict(process_model(config, model).p, dict_factory=enum_dict_factory)
            )

    file_write(
        args.output, json.dumps(output_json, indent=4, sort_keys=False, cls=EtmEncoder)
    )
