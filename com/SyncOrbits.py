...
# app
from geode import pyDate, pyOptions, pyRunWithRetry
from geode.Utils import add_version_argument, process_date, required_length
...
def main():
    ...
    try:
        ...
        pbar = tqdm(
            desc="%-30s" % " >> Synchronizing orbit files",
            total=len(drange),
            ncols=160,
            disable=None,
        )
        ...
        def downloadIfMissing(ftp_list, ftp_filename, local_filename, local_dir, desc):
            ...
            if not os.path.isfile(mark_path) and ftp_filename in ftp_list:
                tqdm.write(
                    "%-31s: %s" % (" -- trying to download " + desc, ftp_filename)
                )
        ...
        for date in (pyDate.Date(mjd=mdj) for mdj in drange):
            ...
            for sp3type in Config.sp3types:
                ...
                tqdm.write(
                    " -- Checking in %s and %s for sp3, clock, and erp files"
                    % (opera_folder, repro_folder)
                )

                for folder, ftp_list in [
                    (opera_folder, opera_list),
                    (repro_folder, repro_list),
                ]:
                    ...
                    try:
                        ...
                        for ext, recmp in [
                            ...
                            for file in match:
                                downloadIfMissing(
                                    ftp_list, file, file, sp3_archive, ext
                                )
                    except ftplib.error_perm:
                        continue
            ...
            try:
                ...
                for ext in (".Z", ".gz"):
                    ...
                    if downloadIfMissing(
                        ftp_list, ftp_filename, filename, brdc_archive, "BRDC"
                    ):
                        # decompress file
                        tqdm.write(
                            "  -> Download succeeded %s"
                            % os.path.join(brdc_archive, ftp_filename)
                        )
                        pyRunWithRetry.RunCommand(
                            "gunzip -f " + os.path.join(brdc_archive, ftp_filename), 15
                        ).run_shell()
            ...
            try:
                ...
                if not (
                    os.path.exists(os.path.join(ionex_archive, l_fname))
                    or os.path.exists(os.path.join(ionex_archive, s_fname))
                ):
                    if downloadIfMissing(
                        ftp_list, l_fname, l_fname, ionex_archive, "IONEX"
                    ):
                        # try long name first
                        tqdm.write(
                            "  -> Download succeeded %s"
                        ...
                        # leave it zipped
                    elif downloadIfMissing(
                        ftp_list, s_fname, s_fname, ionex_archive, "IONEX"
                    ):
                        # try short name
                        tqdm.write(
                            "  -> Download succeeded %s"
                            % os.path.join(ionex_archive, s_fname)
                        )
