    ...
    for stn in stninfo:
        if stn.get("StationCode").lower() == StationCode:
            try:
                ...
                stnobj = pyStationInfo.StationInfo(
                    cnn, NetworkCode, StationCode, allow_empty=True
                )
...
def execute_ppp(record, rinex_path, h_tolerance):
    ...
    try:
        ...
        if ppp_soln.ntuples() == 0:
            ...
            with pyRinex.ReadRinex(NetworkCode, StationCode, rinex_path) as Rinex:
                ...
                with pyPPP.RunPPP(
                    ...
                    hash=stninfo.currentrecord.hash,
                ) as ppp:
                    ppp.exec_ppp()
        ...
        pyPPP.pyRunPPPException,
        pyStationInfo.pyStationInfoException,
    ) as e:
        e.event["StationCode"] = StationCode
        e.event["NetworkCode"] = NetworkCode
            ...
            if not rinex:
                ...
                if stninfo.currentrecord.hash + crc32(soln["orbit"]) != soln["hash"]:
                    if not rehash:
                        ...
                        cnn.update(
                            "ppp_soln",
                            {"hash": stninfo.currentrecord.hash + crc32(soln["orbit"])},
                            **soln,
                        )
...
def db_checks(cnn):
    ...
    if fields.get("hash") and fields["hash"].lower() != "bigint":
        ...
        for rnx in rinex:
            try:
                with pyRinex.ReadRinex(NetworkCode, StationCode, rnx) as rinexinfo:
                    ...
                    try:
                        pyStationInfo.StationInfo(
                            cnn, NetworkCode, StationCode, rinexinfo.date
                        )
                        ...
                        for record in import_stninfo:
                            import_record = pyStationInfo.StationInfoRecord(
                                NetworkCode, StationCode, record
                            )
    ...
    for stn in tqdm(stnlist, ncols=80, disable=None):
        ...
        if rinex is not None:
            ...
            with pyRinex.ReadRinex(NetworkCode, StationCode, rinex, False) as Rinex:  # type: pyRinex.ReadRinex
                StationInfo = pyStationInfo.StationInfo(
                    cnn, NetworkCode, StationCode, Rinex.date
                )
