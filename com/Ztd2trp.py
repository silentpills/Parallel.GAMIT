...
def main():
    parser = argparse.ArgumentParser(
        description="Zenith tropospheric delays to BERNESE TRP format"
    )

    parser.add_argument(
        "project", metavar="project_name", help="GAMIT processing project name"
    )
    ...
    # print options
    print(
        " >> Generating ZTD for project %s between dates %s %s"
        % (args.project, dates[0].yyyyddd(), dates[1].yyyyddd())
    )
    ...
    for x in tqdm(range(dates[0].mjd, dates[1].mjd), ncols=80, disable=None):
        ...
        # execute a statement
        query = (
            'SELECT gamit_ztd."NetworkCode", gamit_ztd."StationCode", gamit_ztd."Date", '
        ...
        filename = os.path.join(
            args.directory, args.project.upper() + dd.wwwwd() + ".TRP"
        )
        ...
        with open(filename, "w") as fp:
            ...
            hdr_1 = (
                "Daily solution (GPS) for "
                + dd.wwwwd()
                + " (CRD+TRP)                     \n"
            )
            hdr_2 = "-".ljust(133, "-") + "\n"
            hdr_3 = (
                " A PRIORI MODEL:  -17   MAPPING FUNCTION:    8   GRADIENT MODEL:    4   MIN. ELEVATION:    10   "
                "TABULAR INTERVAL:  3600 / 86400\n\n"
            )
            hdr_4 = (
                " STATION NAME     FLG   YYYY MM DD HH MM SS   YYYY MM DD HH MM SS   MOD_U   CORR_U  SIGMA_U "
            ...
            for data in rows:
                ...
                date_ = date_.replace(":", " ")
                fp.write(
                    " {stnm}{dome_number}{flag}{date}{spaces}{mod_u} {corr_u}{sigma_u}{total_u}  {corr_n}"
                    "{sigma_n} {corr_e}{sigma_e}\n".format(
                        stnm=data[1].ljust(station_len).upper(),
                        dome_number=dome,
                        flag="A",
