    ...
    def do_smoothing_start_stop(self, arg: str):
        ...
        try:
            ...
            if len(args) == 3:
                ...
                self.etm_stacker.update_smoothing_start_stop(
                    eid, float(value_start), float(value_stop)
                )
                print(
                    f"Updated event {eid} smoothing start {value_start} stop {value_stop}"
                )
                ...
                for event in self.etm_stacker.earthquakes:
                    self.etm_stacker.update_smoothing_start_stop(
                        event.id, float(args[0]), float(args[1])
                    )
                    print(
                        f"Updated event {event.id} smoothing start {args[0]} stop {args[1]}"
                    )
            ...
            print(
                f"Usage: smoothing_start_stop <id> <value_start> <value_stop>: {str(e)}"
            )
