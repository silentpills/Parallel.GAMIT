        ...
        if ts.size:
            if ts.shape[0] > 2:
                ...
                if etm.A is not None:
                    figfile = etm.plot(
                        fileio=io.BytesIO(), plot_missing=False, t_win=pdates
                    )
                    if histogram:
...
def main():
    ...
    JobServer.create_cluster(
        compute_dra, progress_bar=qbar, callback=callback_handler, modules=modules
    )
