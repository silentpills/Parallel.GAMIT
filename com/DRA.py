...
from geode.etm.core import etm_config, etm_engine, type_declarations
from geode.pyDate import Date
from geode.pyStack import Polyhedron, np_array_vertices
from geode.Utils import add_version_argument, file_write, json_converter, process_date
...
def save_excluded_soln(cnn, etm, network_code: str, station_code: str, project: str):
    res_mag = numpy.sqrt(
        numpy.sum(numpy.square([r.residuals for r in etm.fit.results]), axis=0)
    )

    for date, f, r in zip(
        etm.solution_data.coordinates.dates, etm.fit.outlier_flags, res_mag
    ):
        if (
            not cnn.query_float(
                ...
                '"Year" = %i AND "DOY" = %i'
                % (network_code, station_code, project, date.year, date.doy)
            )
            and not f
        ):
            cnn.query(
                'INSERT INTO gamit_soln_excl ("NetworkCode", "StationCode", "Project", '
...
def compute_dra(
    ts, NetworkCode, StationCode, pdates, project, histogram=False, save_excluded=False
):
    try:
        ...
        if ts.size:
            if ts.shape[0] > 2:
                ...
                if etm.config.modeling.status == etm.config.modeling.status.POSTFIT:
                    ...
                    # save the wrms
                    return (
                        etm.fit.results[0].wrms * 1000,
                        etm.fit.results[1].wrms * 1000,
                        etm.fit.results[2].wrms * 1000,
                        fig_file,
                        his_file,
                        filename,
                        NetworkCode,
        ...
        raise Exception(
            "While working on %s.%s" % (NetworkCode, StationCode) + "\n"
        ) from e
...
class DRA(list):
        ...
        for d in tqdm(
            ...
            if i < len(self.dates) - 1:
                if d != self.dates[i + 1] - 1:
                    for dd in [
                        Date(mjd=md)
                        for md in list(range(d.mjd + 1, self.dates[i + 1].mjd))
                    ]:
    ...
    def stack_dra(self):
        for j in tqdm(
            ...
            tqdm.write(
                " -- %s (%04i) %2i it wrms: %4.1f T %6.1f %6.1f %6.1f "
    ...
    def to_json(self, json_file):
        ...
        file_write(
            json_file,
            json.dumps(
                {"transformations": self.transformations, "stats": self.stats},
                indent=4,
                sort_keys=False,
...
def callback_handler(job):
    ...
    if job.exception:
        ...
        if job.result[0] is not None:
            ...
            tqdm.write(
                " -- Station %s.%s did not produce valid statistics"
...
def main():
    ...
    tqdm.write(
        " >> Daily repeatability analysis done. DOYs with wrms > 8 mm are shown below:"
    )
    ...
    qbar = tqdm(
        total=len(dra.stations), desc=" >> Computing DRAs", ncols=160, disable=None
    )
    ...
    ax = axis[0][0]
    ax.plot(
        [t["fyear"] for t in dra.transformations[0]],
    ...
    f.suptitle(
        "Daily repetitivity analysis for project %s\n"
