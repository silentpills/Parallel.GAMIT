...
# app
from geode import dbConnection, pyETM, pyJobServer, pyOptions, pyStack
...
def plot_etm(cnn, stack, station, directory):
    try:
        ...
        ts = pyETM.GamitSoln(
            cnn, ts, station["NetworkCode"], station["StationCode"], stack.project
        )
        etm = pyETM.GamitETM(
            cnn, station["NetworkCode"], station["StationCode"], gamit_soln=ts
        )
...
def station_etm(station, stn_ts, stack_name, iteration=0):
    ...
    try:
        ...
        ts = pyETM.GamitSoln(
            cnn, stn_ts, station["NetworkCode"], station["StationCode"], stack_name
        )

        # create the ETM object
        etm = pyETM.GamitETM(
            cnn, station["NetworkCode"], station["StationCode"], False, False, ts
        )
...
def calculate_etms(
    cnn, stack, JobServer, iterations, create_target=True, exclude_stn=()
):
    """
    ...
    qbar = tqdm(
        total=len(stack.stations) - len(exclude_stn),
        desc=" >> Calculating ETMs",
        ncols=160,
        disable=None,
    )
    ...
    JobServer.create_cluster(
        station_etm, progress_bar=qbar, callback=callback_handler, modules=modules
    )

    # delete all the solutions from the ETMs table
    cnn.query(
        "DELETE FROM etms WHERE \"soln\" = 'gamit' AND \"stack\" = '%s'" % stack.name
    )
    ...
    for station in stack.stations:
        ...
        if stationID(station) in [stationID(s) for s in exclude_stn]:
            tqdm.write(
                " -- Station %s.%s has been manually excluded from the stacking process"
    ...
    if create_target:
        target = []
        for i in tqdm(
            list(range(len(stack.dates))),
            ncols=160,
            desc=" >> Initializing the target polyhedrons",
            disable=None,
        ):
            dd = stack.dates[i]
...
def load_periodic_space(periodic_file):
    ...
    for l in lines:
        if l.startswith("F"):
            ...
            neu = re.findall(
                r"\s(\w+)\s+\w\s.{9}\s*\d*\s(\w)\s+(.{7})\s+.{7}\s+(.{7})", l
            )[0]
            ...
            periods[stn][per][com].append(
                [np.divide(float(neu[3]), 1000.0), np.divide(float(neu[2]), 1000.0)]
            )
        ...
        for per in periods[stn].keys():
            for com in periods[stn][per].keys():
                periods[stn][per][com] = np.mean(
                    np.array(periods[stn][per][com]), axis=0
                ).tolist()
    ...
    with file_open(constrains_file) as f:
        ...
        for s in stn:
            if s[0] in [stationID(stn) for stn in exclude_stn]:
                tqdm.write(
                    " -- Loading constraints: station %s has been manually excluded"
                    % s[0]
                )
            else:
                params[s[0]] = {
                    ...
                    "365.250": {
                        "n": [
                            np.divide(float(s[8]), 1000.0),
                            np.divide(float(s[10]), 1000.0),
                        ],
                        "e": [
                            np.divide(float(s[12]), 1000.0),
                            np.divide(float(s[14]), 1000.0),
                        ],
                        "u": [
                            np.divide(float(s[16]), 1000.0),
                            np.divide(float(s[18]), 1000.0),
                        ],
                    },
                    "182.625": {
                        "n": [
                            np.divide(float(s[9]), 1000.0),
                            np.divide(float(s[11]), 1000.0),
                        ],
                        "e": [
                            np.divide(float(s[13]), 1000.0),
                            np.divide(float(s[15]), 1000.0),
                        ],
                        "u": [
                            np.divide(float(s[17]), 1000.0),
                            np.divide(float(s[19]), 1000.0),
                        ],
                    },
                }
...
def main():
    ...
    if args.external_constraints:
        ...
        if len(constraints) == 0:
            print(
                " >> WARNING: Empty constraints file passed. Check that the lines start with a space."
            )
            exit(1)
        elif len(constraints) < 3:
            print(
                " >> WARNING: A constraints file was passed but %i stations where found. The number of "
                "stations in the constraints file might be insufficient to align all spaces."
                % len(constraints)
            )
    else:
        constraints = None
    ...
    stack = pyStack.Stack(
        cnn, args.project[0], args.stack_name[0], args.redo_stack, end_date=dates[1]
    )
    ...
    for i in range(max_iters):
        ...
        qbar = tqdm(
            total=len(stack),
            ncols=160,
            desc=" >> Aligning polyhedrons (%i of %i)" % (i + 1, max_iters),
            disable=None,
        )
