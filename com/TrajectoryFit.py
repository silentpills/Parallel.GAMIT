...
# app
from geode import Utils, dbConnection, pyETM
from geode.pyETM import CO_SEISMIC_DECAY, CO_SEISMIC_JUMP_DECAY
from geode.Utils import add_version_argument, stationID
...
def generate_kmz(
    kmz, stations, discarded, deformation_type="interseismic", units="mm/yr"
):
    ...
    for s, icon_color, label_scale in (
        (styles_ok.normalstyle, "ff00ff00", 0),
    ...
    for stn in tqdm(
        stations, ncols=160, disable=None, desc=" -- Included station list"
    ):
        stn_id = stationID(stn)
        plot = plot_station_param(
            stn["NetworkCode"],
            stn["StationCode"],
            deformation_type,
            units,
    ...
    for stn in tqdm(
        discarded, ncols=160, disable=None, desc=" -- Excluded station list"
    ):
        stn_id = stationID(stn)
        plot = plot_station_param(
            stn["NetworkCode"],
...
def process_interseismic(
    cnn, stnlist, force_stnlist, stack, sigma_cutoff, vel_cutoff, lat_lim, filename, kmz
):
    ...
    tqdm.write(
        " -- velocity cutoff: %.2f mm/yr; output filename: %s" % (vel_cutoff, filename)
    )
    ...
    for stn in tqdm(stnlist, ncols=160, disable=None):
        try:
            ...
            etm = pyETM.GamitETM(
                cnn, stn["NetworkCode"], stn["StationCode"], stack_name=stack
            )
            ...
            if stn in force_stnlist:
                ...
                if (
                    etm.gamit_soln.lat[0] < lat_lim[0]
                    or etm.gamit_soln.lat[0] > lat_lim[1]
                ):
                    tqdm.write(
                ...
                if (
                    etm.gamit_soln.date[-1].fyear - etm.gamit_soln.date[0].fyear < 2
                    and use
                ):
                    tqdm.write(
                ...
                if etm.A is not None:
                    if len(etm.Jumps.table) > 0 and use:
                        j = next(
                            (
                                ...
                                and j.fit
                                and j.magnitude >= 7
                                and j.date.fyear < etm.gamit_soln.date[0].fyear + 1.5
                            ),
                            None,
                        )
                        if j:
                    ...
                    if (
                        etm.factor[0] * 1000 > sigma_cutoff
                        or etm.factor[1] * 1000 > sigma_cutoff
                    ) and use:
                        tqdm.write(
            ...
            def getvel():
                return {
                    ...
                    "etm": etm.plot(
                        plot_missing=False, plot_outliers=True, fileio=BytesIO()
                    ),
                }
...
def process_postseismic(
    cnn,
    stnlist,
    force_stnlist,
    stack,
    interseimic_filename,
    event,
    filename,
    kmz,
    prev_events=None,
    ...
    if prev_events:
        if np.mod(len(prev_events), 2) != 0:
            raise ValueError(
                "Invalid number of arguments for previous events: needs to be multiple of 2."
            )
    ...
    def getpost():
        return {
            ...
            "lon": etm.gamit_soln.lon[0],
            "n": eq.p.params[0, 0]
            if eq.p.jump_type is CO_SEISMIC_DECAY
    ...
    for stn in tqdm(stnlist, ncols=160, disable=None):
        ...
        try:
            ...
            if prev_events:
                for e, g in zip(pdates, pgrids):
                    ...
                    if np.isnan(pe):
                        ...
                        pn = 0.0
                    postseismic.append(
                        {"date": e, "relaxation": [0.5], "amplitude": [[pn, pe, 0.0]]}
                    )
                    tqdm.write(
                        "    postseismic removal for %s %6.3f %6.3f %6.3f"
                        % (e.yyyyddd(), float(pn), float(pe), 0.0)
                    )
            etm = pyETM.GamitETM(
                cnn,
                stn["NetworkCode"],
            ...
            for eq in [
                e
                for e in etm.Jumps.table
                if e.p.jump_type in (CO_SEISMIC_DECAY, CO_SEISMIC_JUMP_DECAY)
                and e.fit
                and etm.A is not None
            ]:
                ...
                if (
                    eq.date < event
                    and event.fyear - eq.min_date < 2.5
                    and eq.date not in pdates
                ):
                    previous_eq = eq.date
                ...
                if eq.date == event:
                    ...
                    if (
                        eq.constrain_years >= 2.5
                        and eq.constrain_data_points >= eq.constrain_years * 5
                        and not previous_eq
                        ...
                        if eq.constrain_years < 2.5:
                            tqdm.write(
                                "    co-seismic decay not added (less than 2.5 years after event)"
                            )
                        elif eq.constrain_data_points < eq.constrain_years * 5:
                            tqdm.write(
                                "    co-seismic decay not added (too few data points)"
                            )
                        elif previous_eq:
                            tqdm.write(
                                "    co-seismic decay not added (postseismic of event on %s not corrected)"
                                % previous_eq
                            )
...
def main():
    ...
    force_stnlist = Utils.process_stnlist(
        cnn, args.force_stations, summary_title="Forced station list:"
    )
    ...
    if args.postseismic_process is not None:
        process_postseismic(
            ...
            args.postseismic_process[2],
            args.postseismic_process[3],
            args.postseismic_process[4:],
            0,
            0,
        )
