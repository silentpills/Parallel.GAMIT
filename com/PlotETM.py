...
def main():
    ...
    for stn in stnlist:
        ...
        if not args.filename:
            solution_options.solution_type = (
                SolutionType.PPP if args.solution == "ppp" else SolutionType.GAMIT
            )
        ...
        try:
            if from_kmz:
                ...
                print(
                    "About to process " + stn["NetworkCode"] + "." + stn["StationCode"]
                )
                config = EtmConfig(
                    stn["NetworkCode"],
                    stn["StationCode"],
            ...
            config.plotting_config.plot_residuals_mode = (
                "residuals" in args.plot_options
            )
            ...
            config.modeling.least_squares_strategy.adjustment_model = (
                ADJUSTMENT_MODEL_MAP[args.least_squares_strategy]
            )
            ...
            config.modeling.least_squares_strategy.constraints = get_constraints(
            ...
            if isinstance(args.post_seismic_back, str) and any(
                char in args.post_seismic_back for char in ("_", ".", "/")
            ):
            ...
            print(
                "Successfully plotted " + stn["NetworkCode"] + "." + stn["StationCode"]
            )
