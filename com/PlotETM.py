...
def gamit_soln(args, cnn, stn):
    ...
    soln = pyETM.GamitSoln(
        cnn, polyhedrons, stn["NetworkCode"], stn["StationCode"], args.gamit[0]
    )
...
def from_file(args, cnn, stn, placemarks):
    ...
    filename = args.filename.replace("{net}", stn["NetworkCode"]).replace(
        "{stn}", stn["StationCode"]
    )
    ...
    if args.override_database:
        ...
        soln = pyETM.ListSoln(
            cnn, polyhedrons.tolist(), stn["NetworkCode"], stn["StationCode"]
        )

    etm = pyETM.FileETM(
        cnn, soln, False, args.no_model, args.remove_jumps, args.remove_polynomial
    )
...
def main():
    ...
    parser.add_argument(
        ...
        "--plot_missing_data",
        action="store_true",
        help="Show missing days as magenta lines in the plot",
        default=False,
    )
    ...
    parser.add_argument(
        "-hist", "--histogram", action="store_true", help="Plot histogram of residuals"
    )
    ...
    if args.override_database:
        ...
        stnlist = [
            {"NetworkCode": stnm.split(".")[0], "StationCode": stnm.split(".")[1]}
            for stnm, _ in placemarks
        ]
    ...
    if args.time_window is not None:
        if len(args.time_window) == 1:
            try:
                dates = process_date(
                    args.time_window, missing_input=None, allow_days=False
                )
                dates = (dates[0].fyear,)
            except ValueError:
                # an integer value
        ...
        for stn in stnlist:
            try:
                ...
                if args.interactive:
                    ...
                    xfile = os.path.join(
                        args.directory,
                        "%s.%s_%s" % (etm.NetworkCode, etm.StationCode, postfix),
                    )
                ...
                if args.json is not None:
                    ...
                    file_write(
                        xfile + ".json", json.dumps(obj, indent=4, sort_keys=False)
                    )
                ...
                if args.query is not None:
                    ...
                    xyz, _, _, txt = etm.get_xyz_s(
                        q_date.year, q_date.doy, force_model=model
                    )
                    ...
                    print(
                        " %s.%s %14.5f %14.5f %14.5f %8.3f %s -> %s"
                        % (
                            etm.NetworkCode,
                            etm.StationCode,
                            xyz[0],
