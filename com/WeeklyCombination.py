...
# deps
# app
from geode import Utils, dbConnection, pyDate, pyGamitConfig, snxParse
...
def replace_in_sinex(sinex, observations, unknowns, new_val):
    new_unknowns = """ NUMBER OF UNKNOWNS%22i
...
def process_sinex(cnn, project, dates, sinex):
    ...
    zg = cnn.query_float(
        ...
        % (
            project,
            dates[0].first_epoch("fyear"),
            dates[1].last_epoch("fyear"),
            stnlist,
        )
    )
    ...
    zd = cnn.query_float(
        'SELECT count("ZTD") + %i as implicit FROM gamit_ztd '
    ...
    rs = cnn.query(
        'SELECT "NetworkCode", "StationCode", dome FROM stations '
...
class Globk:
    ...
    def create_combination_script(self, org, gpsweek, gpsweekday, sites):
        ...
        site_list_string = []
        for s in sites:
            site_list_string.append(
                'echo " use_site %s"                                     >> globk.cmd'
                % s
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Program to perform weekly loosely-constrained solutions. Combination "
    ...
    print(
        " >> Working with GPS week "
        + args.gpsweek[0]
        + " (%s to %s)" % (date_s.yyyyddd(), date_e.yyyyddd())
    )
    ...
    if not len(stnlist):
        print(
            " >> ERROR: no stations selected. Please check the station list argument."
        )
        exit(1)
    ...
    for i in range(len(stnlist) - 1):
        for j in range(i + 1, len(stnlist)):
            if stnlist[i]["StationCode"] == stnlist[j]["StationCode"]:
                print(
                    "During station selection, two identical station codes were found. "
    ...
    print(
        " >> REMINDER: To automatically remove outliers during the weekly combination, "
    ...
    for day in range(0, 7):
        ...
        soln_dir = os.path.join(
            soln_pwd, "%s/%s/%s/glbf" % (date.yyyy(), date.ddd(), project)
        )
        ...
        if os.path.exists(soln_dir):
            ...
            print(
                " -- Found GLX %s for gps week %i day %i"
                % (glx[0], date.gpsWeek, date.gpsWeekDay)
            )
        else:
            print(
                " -- WARNING: Could not find GLX for gps week %i day %i"
                % (date.gpsWeek, date.gpsWeekDay)
            )
    ...
    for day, glx in enumerate(glx_list):
        ...
        move(
            globk_pwd + "/" + org + date.wwww() + "%i.snx" % (date.gpsWeekDay + 8),
    ...
    process_sinex(
        cnn, project, [date_s, date_e], globk_pwd + "/" + org + date_s.wwww() + "7.snx"
    )
