...
from geode.cluster import BisectingQMeans, overcluster, prune, select_central_point
from geode.plots import plot_geographic_cluster_graph
...
def generate_kmz(OC, lla, stations, central_points, filename):
    ...
    for i in np.arange(0, OC.shape[0], 1):
        ...
        for lat, lon, stnm, tie in zip(
            lla[idx, 0], lla[idx, 1], stations[idx], OC.sum(axis=0)[idx]
        ):
            ...
            line = folder_net.newlinestring(
                name="%s - %s" % (cstn, stnm), coords=[(lon, lat), (clon, clat)]
            )
...
def main():
    ...
    parser.add_argument(
        "-vb", "--verbose", action="store_true", default=False, help="Verbose mode."
    )
    ...
    try:
        if args.date_filter:
            ...
            dates = [
                Date(year=args.year[0], doy=1),
                Date(year=args.year[0] + 1, doy=1) - 1,
            ]
        else:
    ...
    for mjd in tqdm(mjds, ncols=120):
        ...
        points = np.array(
            [
                ...
                for stn in rs
                if stn["ObservationYear"] == date.year
                and stn["ObservationDOY"] == date.doy
            ]
        )
        ...
        if points.shape[0] > 50:
            if not args.deterministic:
                ...
                central_points_ids = select_central_point(
                    points, qmean.cluster_centers_
                )

                dc = DeterministicClustering(
        ...
        A = np.column_stack(
            (np.ones_like(cluster_sizes), cluster_sizes, cluster_sizes**2)
        )
        ...
        if args.verbose:
            ...
            tqdm.write(
                " -- %s C %3i - S: %4i - US: %4i - avg R: %5.3f - ties: [%2i %4.1f %2i] - T: %.1f"
                % (
                    date.yyyyddd(),
                    OC.shape[0],
                    int(np.sum(OC.flatten())),
                    OC.shape[1],
                    R,
                    np.min(tc),
                    np.mean(tc, dtype=float),
                    np.max(tc),
                    T.sum(),
                )
            )
        ...
        if date in plot_dates and points.shape[0] > 50:
            generate_kmz(
                OC, lla, stnm, central_points_ids, date.yyyyddd().replace(" ", "_")
            )
            plot_geographic_cluster_graph(
                points[central_points_ids], cluster_ids, tie_ids, stnm, points
            )

    # estimate the runtime
    tqdm.write(" >> Total estimated time:")
    tqdm.write(
        " -- %s - %s processing time: %.1f hours"
        % (dates[0].yyyyddd(), dates[1].yyyyddd(), np.sum(np.array(t)) / 60.0)
    )
    tqdm.write(
        " -- Wall time using %i cores: %.1f hours"
        % (args.core_count[0], np.sum(np.array(t)) / args.core_count[0] / 60.0)
    )

    if args.export_json:
        file_write(
            args.export_json[0] + ".json",
            json.dumps(
                {"mjd": x, "station_per_clusters": y}, indent=4, sort_keys=False
            ),
        )
    ...
    if args.plots:
        ...
        counts, xedges, yedges, im = axs[0, 0].hist2d(
            x,
            y,
            bins=[np.arange(dates[0].mjd, dates[1].mjd + 2, 1), np.arange(5, 60)],
            cmap="viridis",
        )
        ...
        axs[1, 1].set_title(
            "(d) Number of stations / unique", fontsize=14, fontweight="bold"
        )

        # Layout
        plt.suptitle(
            "Target size: %i ties: %i" % (args.target_size[0], args.tie_count[0])
        )
        plt.tight_layout()
        plt.show()
