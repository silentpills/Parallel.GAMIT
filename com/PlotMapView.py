...
# app
from geode.pyETM import CO_SEISMIC_DECAY, CO_SEISMIC_JUMP_DECAY, LABEL
...
def plot_map_view(pngfile, etm, lneu, fil, event, co_jump):
    ...
    f.suptitle(
        LABEL("station") + " %s (%s %.2f%%) lat: %.5f lon: %.5f - Event: %s\n"
        "%s\n%s\n"
        ...
        % (
            ...
            etm.soln.lon,
            event.yyyyddd(),
            etm.Linear.print_parameters(
                np.array([etm.soln.auto_x, etm.soln.auto_y, etm.soln.auto_z]),
                etm.soln.lat,
                etm.soln.lon,
            ),
...
def main():
    ...
    def getpost():
        ...
        return {
            ...
            "map": plt,
            "etm": etm.plot(
                plot_missing=args.plot_missing_solutions,
                plot_outliers=True,
                fileio=BytesIO(),
            ),
            "models": app_models,
    ...
    for stn in tqdm(stnlist, ncols=160, disable=None):
        ...
        try:
            ...
            if prev_events:
                ...
                for e, g in zip(pdates, pgrids):
                    ...
                    def apply_model():
                        ...
                        tqdm.write(
                            "    postseismic removal for %s %6.3f %6.3f %6.3f using model"
                            % (e.yyyyddd(), float(pn), float(pe), 0.0)
                        )
                        app_models.append(
                            "%s using model %6.3f %6.3f %6.3f for postseismic"
                            % (e.yyyyddd(), float(pn), float(pe), 0.0)
                        )
                    ...
                    if prev_event_j:
                        ...
                        if prev_event_j[0].p.jump_type == CO_SEISMIC_DECAY:
                            ...
                            tqdm.write(
                                "    postseismic removal for %s using ETM" % e.yyyyddd()
                            )
                            app_models.append(
                                "%s using ETM for postseismic" % e.yyyyddd()
                            )
                    else:
            ...
            e = [
                ...
                if j.p.jump_type in TYPES and j.fit and j.date == event
            ]
            ...
            if e:
                ...
                for j in [
                    ...
                    if j.p.jump_type is CO_SEISMIC_JUMP_DECAY and (
                        j.date == event or j.date in epdates
                    ):
                        ...
                        mj = mj + np.array(
                            [
                                (np.dot(b[:, 0], j.p.params[i, 0])) * 1000
                                for i in range(3)
                            ]
                        )
                        ...
                        # tqdm.write(str(j.p.params))
                        oj = oj + np.array(
                            [(np.dot(a, j.p.params[i])) * 1000 for i in range(3)]
                        )
                        mj = mj + np.array(
                            [(np.dot(b, j.p.params[i])) * 1000 for i in range(3)]
                        )
                    else:
                ...
                op = np.array(
                    [
                        (
                            np.dot(
                                etm.Linear.get_design_ts(etm.soln.t),
                                etm.Linear.p.params[i],
                            )
                        )
                        * 1000
                        for i in range(3)
                ...
                mp = np.array(
                    [
                        (
                            np.dot(
                                etm.Linear.get_design_ts(etm.soln.ts),
                                etm.Linear.p.params[i],
                            )
                        )
                        * 1000
                ...
                if etm.Periodic.frequency_count > 1:
                    of = np.array(
                        [
                            (
                                np.dot(
                                    etm.Periodic.get_design_ts(etm.soln.t),
                                    etm.Periodic.p.params[i],
                                )
                            )
                            * 1000
                            for i in range(3)
                    ...
                    mf = np.array(
                        [
                            (
                                np.dot(
                                    etm.Periodic.get_design_ts(etm.soln.ts),
                                    etm.Periodic.p.params[i],
                                )
                            )
                            * 1000
                ...
                m = np.array(
                    [
                        (np.dot(etm.As, etm.C[i])) * 1000 - mj[i] - mp[i] - mf[i]
                        for i in range(3)
                    ]
                )
                ...
                if e[0].p.jump_type == CO_SEISMIC_DECAY:
                    ...
                    data = (
                        ...
                        - oj
                        - of
                        - np.tile(m[:, 0], (oj.shape[1], 1)).transpose()
                    )
                ...
                if cgrid is not None and e[0].p.jump_type == CO_SEISMIC_DECAY:
                    ...
                    app_models.append(
                        "%s using modeled coseismic component NEU = (%6.3f %6.3f %6.3f) for event"
                        % (
                            e[0].date.yyyyddd(),
                            float(co_jump[0]),
                            float(co_jump[1]),
                    ...
                    app_models.append(
                        "%s using observed coseismic component NEU = (%6.3f %6.3f %6.3f) for event"
                        % (
                            e[0].date.yyyyddd(),
                            float(co_jump[0]),
                            float(co_jump[1]),
    ...
    generate_kmz(
        os.path.join(
            args.directory, "mapviews_%s_%03i.kmz" % (event.yyyy(), event.doy)
        ),
        map_views,
    )
