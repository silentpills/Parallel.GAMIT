...
from geode.pyRinexName import RinexNameFormat
...
def compare_stninfo_rinex(NetworkCode, StationCode, STime, ETime, rinex_serial):
    try:
        ...
        return (
            traceback.format_exc() + " open de database when processing "
            "processing %s.%s" % (NetworkCode, StationCode),
            None,
        )
    ...
    try:
        ...
        stninfo = station_info.StationInfo(cnn, NetworkCode, StationCode, date)
...
def get_differences(differences):
    ...
    return sorted(
        (diff[1] for diff in differences if diff[1] is not None),
...
def check_rinex_integrity(
    cnn, Config, stnlist, start_date, end_date, operation, JobServer
):
    global differences
...
def stn_info_rinex_integrity(cnn, stnlist, start_date, end_date, JobServer):
    ...
    JobServer.create_cluster(
        compare_stninfo_rinex, callback=stnrnx_callback, modules=modules
    )
    ...
    for stn in stnlist:
        ...
        tqdm.write(
            ...
            "check for %s.%s..." % (NetworkCode, StationCode),
            file=sys.stderr,
        )
        maxlen = 0
        ...
        for i, diff in enumerate(diff_vect):
            ...
            if i != len(diff_vect) - 1:
                if (
                    ...
                    sys.stdout.write(
                        "%s %s: RINEX SN %s != Station Information %s Possible change in station or "
                        "bad RINEX metadata.\n"
                        % (
                            year,
                            doy,
...
def stn_info_check(cnn, stnlist, Config):
    ...
    for stn in stnlist:
        ...
        first_obs = False
        try:
            stninfo = station_info.StationInfo(cnn, NetworkCode, StationCode)  # type: StationInfo
            ...
            if len(empty_edata) > 1:
                list_empty = ", ".join(
                    pyDate.Date(datetime=record[1]).yyyyddd() for record in empty_edata
                )
                sys.stdout.write(
            ...
            if (
                len(empty_edata) > 1
                or len(list_problems) > 0
                or first_obs
                or len(station_list_gaps) > 0
                or atx_problem
            ):
    ...
    for stn in stnlist:
        ...
        if rs.ntuples() > 0:
            ...
            for soln in tqdm(ppp):
                ...
                lat, lon, h = ecef2lla(
                    [float(soln["X"]), float(soln["Y"]), float(soln["Z"])]
                )
            ...
            tqdm.write(
                "\nNo PPP solutions found for %s.%s..." % (NetworkCode, StationCode),
                sys.stderr,
            )
...
def GetGaps(cnn, NetworkCode, StationCode, start_date, end_date):
    ...
    if len(rnxtbl) > 0:
        start_date = pyDate.Date(
            year=rnxtbl[0]["ObservationYear"], doy=rnxtbl[0]["ObservationDOY"]
        )
        end_date = pyDate.Date(
            year=rnxtbl[-1]["ObservationYear"], doy=rnxtbl[-1]["ObservationDOY"]
        )

        possible_doys = [
            pyDate.Date(mjd=mjd) for mjd in range(start_date.mjd, end_date.mjd + 1)
        ]
        ...
        gaps = [doy for doy in possible_doys if doy not in actual_doys]
...
def RinexCount(cnn, stnlist, start_date, end_date):
    ...
    rs = cnn.query(
        ...
        'ORDER BY "ObservationSTime"' % (start_date.yyyymmdd(), end_date.yyyymmdd())
    )
    ...
    for stn in stnlist:
        ...
        for i, rnx in enumerate(rnxtbl):
            if i > 0:
                ...
                d2 = pyDate.Date(
                    year=rnxtbl[i - 1]["ObservationYear"],
...
def PrintStationInfo(cnn, stnlist, short=False):
    for stn in stnlist:
        ...
        try:
            stninfo = station_info.StationInfo(cnn, NetworkCode, StationCode)
...
def rename_station(
    ...
    try:
        ...
        if rs.ntuples() == 0:
            ...
            print(
                "The requested destiny station does not exist. Please create it and try again"
            )
        else:
            ...
            print(
                " >> Beginning transfer of %i rinex files from %s.%s to %s.%s"
                % (
                    len(original_rs),
                    NetworkCode,
                    StationCode,
            ...
            for src_rinex in tqdm(original_rs):
                ...
                date = pyDate.Date(
                    year=int(src_rinex["ObservationYear"]),
                ...
                try:
                    ...
                    dest_path = src_path.replace(StationCode, DestStationCode).replace(
                    ...
                    shutil.move(
                        os.path.join(src_path, src_file),
                    ...
                    event = pyEvents.Event(
                        Description=f"RINEX file {rnx} from {NetworkCode}.{StationCode} was merged "
                    ...
                    event = pyEvents.Event(
                        Description="A new RINEX (%s) was merged to the archive from %s.%s"
                        % (
                            rinex_name.to_rinex_format(
                                pyRinexName.TYPE_RINEX, no_path=True
                            ),
                    ...
                    tqdm.write(
                        f" -- RINEX file {src_file} could not be moved because it already exists in "
                    ...
                    rnx = src_rinex["Filename"]
                    event = pyEvents.Event(
                        Description=f"RINEX file {rnx} from {NetworkCode}.{StationCode} was deleted "
                ...
                try:
                    ...
                    # the source station
                    try:
                        stninfo_dest = station_info.StationInfo(
                            cnn, DestNetworkCode, DestStationCode
                        )
                        stninfo_src = station_info.StationInfo(
            ...
            if delete_if_empty:
                ...
                if len(original_rs) == 0:
                    ...
                    stninf = station_info.StationInfo(
                    ...
                    for table in tables:
                        cnn.query(
                            "DELETE FROM %s WHERE \"NetworkCode\" = '%s' AND \"StationCode\" = '%s'"
                    ...
                    tqdm.write(
                        " -- Could not delete station: %i remaining RINEX files"
                        % (len(original_rs))
                    )
...
def visualize_gaps(cnn, stnlist, start_date, end_date):
    for Stn in stnlist:
        ...
        missing_doys, possible_doys = GetGaps(
            cnn, Stn["NetworkCode"], Stn["StationCode"], start_date, end_date
        )
        ...
        sys.stdout.write(
            " -- %s: (First and last observation in timespan: %i %03i - %i %03i)\n"
            % (
                stn_id,
                possible_doys[0].year,
                possible_doys[0].doy,
                possible_doys[-1].year,
                possible_doys[-1].doy,
            )
        )
        ...
        for year in sorted({d.year for d in possible_doys}):
            ...
            if len(p_doys) / 2.0 > 120:
                cut_len = int(ceil(len(p_doys) / 4.0))
            else:
                cut_len = len(p_doys)
    ...
    for stn in stnlist:
        ...
        for soln in tqdm(ppp):
            try:
                ...
                tqdm.write(
                    "PPP solution for %i %i is already in the excluded solutions table\n"
                    % (soln["Year"], soln["DOY"])
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Database integrity tools, metadata check and fixing tools program"
    )
    ...
    if args.check_rinex:
        check_rinex_integrity(
            cnn, Config, stnlist, dates[0], dates[1], args.check_rinex[0], JobServer
        )
