...
def getTimeSegments2(starttime, endtime):
    ...
    if len(starts) != len(ends):
        raise IndexError(
            'Number of time segment starts/ends does not match for times: "%s" and "%s"'
            % (starttime, endtime)
        )
...
class AddEarthquakes:
    def __init__(self, cnn, stime=None, etime=None):
        ...
        for stime, etime in tqdm(segments, ncols=120):
            ...
            while retry < 3:
                try:
                    eventlist = libcomcat.search.search(
                        starttime=stime, endtime=etime, minmagnitude=6
                    )
                    break
                except ConnectionError as e:
                    retry += 1
                    tqdm.write(
                        "There was a connection error while downloading segment %s %s, retrying %i, %s"
            ...
            for event in eventlist:
                ...
                rs_event = cnn.query(
                    "SELECT * FROM earthquakes WHERE id = '%s'" % event.id
                )

                if not len(rs_event.dictresult()):
                    try:
                        # DDG: try to get the moment-tensor solution, if it exists
                        mt = libcomcat.search.get_product_bytes(
                            event.id, "moment-tensor", "quakeml.xml"
                        ).decode("utf-8")
                        # convert from XML to DICT
                        mtp = xmltodict.parse(mt)

                        nodal_planes = mtp["q:quakeml"]["eventParameters"]["event"][
                            "focalMechanism"
                        ]["nodalPlanes"]
                        ...
                        tqdm.write(
                            "Event id %s (%s mag=%.1f) has no moment-tensor solution: %s"
                    ...
                    tqdm.write(
                        "event %s %s already exists in the database"
