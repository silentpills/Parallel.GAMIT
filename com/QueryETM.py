...
# app
from geode import Utils, dbConnection, pyDate, pyETM
...
def main():
    parser = argparse.ArgumentParser(
        description="Query ETM for stations in the database. Default is PPP ETMs."
    )
    ...
    parser.add_argument(
        "-vel", "--velocity", action="store_true", help="Output the velocity in XYZ."
    )
    ...
    if len(args.stnlist) == 1 and os.path.isfile(args.stnlist[0]):
        ...
        stnlist = [
            {"NetworkCode": items[0], "StationCode": items[1]}
            for items in (
    ...
    for stn in stnlist:
        try:
            if args.gamit is None and args.filename is None:
                ...
                etm = pyETM.GamitETM(
                    cnn, stn["NetworkCode"], stn["StationCode"], False, gamit_soln=soln
                )
            ...
            if args.query is not None:
                ...
                if args.velocity and etm.A is not None:
                    ...
                    strp = "%8.5f %8.5f %8.5f " % (vxyz[0, 0], vxyz[1, 0], vxyz[2, 0])

                # also output seasonal terms, if requested
                if args.seasonal_terms and etm.Periodic.frequency_count > 0:
                    strp += " ".join(
                        "%8.5f" % (x * 1000) for x in etm.Periodic.p.params.flatten()
                    )
