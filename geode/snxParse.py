...
# app
from .Utils import file_open, get_norm_year_str
...
class mergedSinexStationData:
    ...
    def Print(self):
        ...
        if self.snxStnData.X != None:
            print(
                "%13.4f %13.4f %13.4f "
                % (self.snxStnData.X, self.snxStnData.Y, self.snxStnData.Z),
                end=" ",
            )
    ...
    def parse(self):
        ...
        if self.snxFilePath != None:
            ...
            try:
                ...
                print(
                    "snxFileParser ERROR:  Could not open file "
                    + self.snxFilePath
                    + " !!!"
                )
                raise
            ...
            with snxFileHandle:
                ...
                #
                #
                varianceFactorPattern = re.compile(
                    r"^ VARIANCE FACTOR\s+([\d+]?\.\d+)$"
                )
                ...
                startSolutionMatrixEstimate = re.compile(
                    r"^\+SOLUTION\/MATRIX_ESTIMATE.*"
                )

                # make pattern to look for the L COVA end tag (-SOLUTION/MATRIX_ESTIMATE L COVA)
                endSolutionMatrixEstimate = re.compile(
                    r"^\-SOLUTION\/MATRIX_ESTIMATE.*"
                )
                ...
                stationCoordinatePattern = re.compile(
                    r"^\s+(\d+)+\s+STA(\w)\s+(\w+)\s+(\w).*\d+\s+(-?[\d+]?\.\d+[Ee][+-]?\d+)\s+(-?[\d+]?\.\d+[Ee][+-]?\d+)$"
                )
                ...
                stationVelocityPattern = re.compile(
                    r"^\s+\d+\s+VEL(\w)\s+(\w+)\s+\w\s+....\s+(\d\d:\d\d\d).*\d+\s+(-?[\d+]?\.\d+[Ee][+-]?\d+)\s+(-?[\d+]?\.\d+[Ee][+-]?\d+)$"
                )
                ...
                for line in snxFileHandle:
                    ...
                    if inSolutionEstimateSection:
                        ...
                        if stationCoordinateMatch:
                            (ID, coordID, stationName, pointCode, coord, sigCoord) = (
                                stationCoordinateMatch[0]
                            )

                            if pointCode != "A":
                                os.sys.stderr.write(
                                    "ignoring solution/estimate STA"
                                    + coordID
                                    + " for station: "
                                    + stationName
                                    + ", point code = "
                        ...
                        if stationVelocityMatch:
                            (coordID, stationName, refEpoch, vel, sigVel) = (
                    ...
                    if inSolutionMatrixEstimateSection:
                        ...
                        if ID1 in list(stn_ID.keys()):
                            for i, ID2 in enumerate(range(len(matrixLine) - 2)):
                                ...
                                if (
                                    stn_ID[ID1] == stn_ID[ID2]
                                    and dictID[ID1] != dictID[ID2]
                                ):
                                    # we already have the variance, we want the covariance
                                    if (dictID[ID1] == "X" and dictID[ID2] == "Y") or (
                                        dictID[ID1] == "Y" and dictID[ID2] == "X"
                                    ):
                                        self.stationDict[stn_ID[ID1]].sigXY = float(
                                            matrixLine[i + 2]
                                        )

                                    elif (
                                        dictID[ID1] == "X" and dictID[ID2] == "Z"
                                    ) or (dictID[ID1] == "Z" and dictID[ID2] == "X"):
                                        self.stationDict[stn_ID[ID1]].sigXZ = float(
                                            matrixLine[i + 2]
                                        )

                                    elif (
                                        dictID[ID1] == "Y" and dictID[ID2] == "Z"
                                    ) or (dictID[ID1] == "Z" and dictID[ID2] == "Y"):
                                        self.stationDict[stn_ID[ID1]].sigYZ = float(
                                            matrixLine[i + 2]
                                        )
...
class snxStationMerger:
    ...
    def addStation(self, level, orgName, stationName, snxStnData):
        ...
        self.mergedStationDict[level][stationName] = mergedSinexStationData(snxStnData)
    ...
    def addStationsFromSinexObject(self, snxObj):
        ...
        for stationName in snxObj.stationDict.keys():
            for level in self.mergedStationDict.keys():
                ...
                # station with same name should be compared by coordinate(s)
                elif self.compareUsingCoordinates(
                    ...
                    self.mergedStationDict[level][stationName].orgNameSet.add(
                        snxObj.orgName
                    )
...
def main():
    ...
    for stn in snxParser:
        ...
        xyzStr = (
            "%9s %4s %9s     GPS %4s                  %12.3f %12.3f %12.3f %5.3f %5.3f %5.3f OSU"
            % (
                stnData.domesNumber,
                stn,
                stnData.domesNumber,
        ...
        velStr = (
            "%9s %4s %9s         %4s                  %12.3f %12.3f %12.3f %5.3f %5.3f %5.3f OSU"
