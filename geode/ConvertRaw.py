...
# app
from . import Utils, pyRinex, pyRunWithRetry
...
class ConvertRaw(object):
    def __init__(
        ...
        self.logger.append(
            Event(
                Description="Initialized ConvertRaw for station code %s with path %s "
        ...
        if os.path.isdir(path_to_raw):
            ...
            self.file_list = list(
                glob.iglob("**/*.*", root_dir=path_to_raw, recursive=True)
            )
        else:
            # single file
        ...
        self.logger.append(
            Event(Description="List of files to process: " + ", ".join(self.file_list))
        )
        ...
        if atx_file is not None and antenna is not None:
            ...
            if not ant or len(ant) > 1:
                ...
                self.logger.append(
                    Event(
                        Description="Replacing ANT # / TYPE record with provided information:"
    ...
    def process_files(self):
        ...
        for file in self.file_list:
            ...
            # OTHER CONVERSIONS COMING SOON
                ...
                self.logger.append(
                    Event(
                        Description="Raw format not supported: %s" % file[-3:].upper()
                    )
                )
    ...
    def merge_rinex(self):
        ...
        for file in file_list:
            ...
            if rnx.date.yyyyddd() + f"_{rnx.interval:>02.0f}" in date_dict.keys():
                ...
                date_dict[rnx.date.yyyyddd() + f"_{rnx.interval:>02.0f}"] = [
                    abs_filename
                ]
        ...
        for d, files in date_dict.items():
            if len(files) > 1:
                rnx = []
                self.logger.append(
                    Event(Description="Processing date / interval %s" % d)
                )
                for frnx in files:
                    ...
                    rnx.append(
                        pyRinex.ReadRinex(
                            "???", self.station_code, frnx, min_time_seconds=300
                        )
                    )
                    os.remove(frnx)
                ...
                cmd = pyRunWithRetry.RunCommand(
                    "gfzrnx_lx -finp %s -fout %s -vo %i" % (fs, spliced_rnx, 2), 300
                )
                _, _ = cmd.run_shell()

                rx = pyRinex.ReadRinex(
                    "???", self.station_code, spliced_rnx, min_time_seconds=300
                )
    ...
    def convert_trimble(self, filename):
        ...
        tmp_dir = tempfile.mkdtemp(
            suffix=".tmp", prefix=os.path.join(self.out_path, "process.")
        )
        ...
        self.logger.append(
            Event(Description="Executed runpkr00 -g -d %s %s" % (t0_file, tmp_dir))
        )
        ...
        self.logger.append(
            Event(Description="Executed teqc -tr d %s > %s 2> %s" % (tgd, rinex, err))
        )

        try:
            rnx = pyRinex.ReadRinex(
                "???", self.station_code, rinex, min_time_seconds=300
            )

            rnx.rename(
                self.station_code + rnx.date.ddd() + "0." + rnx.date.yyyy()[2:] + "o"
            )

            if self.antenna:
                header = rnx.replace_record(
                    rnx.header,
                    "ANT # / TYPE",
                    [
                        self.antenna_serial
                        if self.antenna_serial is not None
                        else rnx.antNo,
                        "%-16s%4s" % (self.antenna, "NONE"),
                    ],
                )
                header = rnx.insert_comment(
                    header, "ConvertRaw REPLACED ANT %s" % rnx.antType
                )
