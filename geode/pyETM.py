...
from . import dbConnection, pyDate, pyEvents, pyOkada

# app
from .metadata.station_info import StationInfo, StationInfoException
from .pyBunch import Bunch
from .Utils import crc32, ct2lg, lg2ct, lla2ecef, rotlg2ct, stationID
    ...
    def __init__(self, m_type, **kwargs):
        ...
        if m_type == self.LOG:
            ...
            if self.log_amplitude.shape[0] != self.relaxation.shape[0]:
                raise pyETMException_Model(
                    "'log_amplitude' dimension 0 must match the elements in relaxation"
                )
    ...
    def eval(self, t):
        ...
        for i in range(3):
            if self.type == self.VEL:
                logger.info(
                    "Applying velocity for reference date %s" % self.date.yyyyddd()
                )
                ...
                for j, r in enumerate(self.relaxation):
                    ...
                    hl[t > self.date.fyear] = np.log10(
                        1.0 + (t[t > self.date.fyear] - self.date.fyear) / r
                    )
    ...
    if isinstance(dictionary, dict):
        for key, val in list(dictionary.items()):
            if isinstance(val, np.ndarray):
                dictionary[key] = (
                    str(val.flatten().tolist()).replace("[", "{").replace("]", "}")
                )
            if key in "covar":
        ...
        dictionary = (
            str(dictionary.flatten().tolist()).replace("[", "{").replace("]", "}")
        )
    ...
    def __init__(self, cnn, NetworkCode, StationCode):
        ...
        if stn["lat"] is None:
            raise pyETMException(
                "Station %s has no valid metadata in the stations table." % stn_id
            )
        ...
        self.table = [
            ...
            if np.sqrt(
                np.square(item[0] - x) + np.square(item[1] - y) + np.square(item[2] - z)
            )
            <= self.max_dist
            and item[3:] not in self.excluded
        ]
        ...
        self.ts_blu = np.array(
            [pyDate.Date(year=item[3], doy=item[4]).fyear for item in self.blunders]
        )
        ...
        if self.solutions >= 1:
            ...
            self.z = a[:, 2]
            self.t = np.array(
                [pyDate.Date(year=item[0], doy=item[1]).fyear for item in a[:, 3:5]]
            )
            self.mjd = np.array(
                [pyDate.Date(year=item[0], doy=item[1]).mjd for item in a[:, 3:5]]
            )
            ...
            raise pyETMException(
                ...
                % (stn_id, np.array([item[5] for item in self.blunders]).min())
            )
        else:
        ...
        self.completion = (
            100.0
            - float(len(self.ts_ns)) / float(len(self.ts_ns) + len(self.t)) * 100.0
        )
        ...
        self.hash = crc32(
            str(len(self.t) + len(self.blunders))
            + " "
            + str(self.auto_x)
            + str(self.auto_y)
            + str(self.auto_z)
        ...
        if stn["lat"] is not None:
            ...
            if self.solutions >= 1:
                        ...
                        np.sqrt(
                            np.sum(
                                np.square(
                                    a[:, 0:3]
                                    - np.array(
                                        [stn["auto_x"], stn["auto_y"], stn["auto_z"]]
                                    )
                                ),
                                axis=1,
                            )
                ...
                if np.any(nb):
                    ...
                    self.t = np.array(
                        [
                            pyDate.Date(year=item[0], doy=item[1]).fyear
                            for item in a[nb, 3:5]
                        ]
                    )
                    self.mjd = np.array(
                        [
                            pyDate.Date(year=item[0], doy=item[1]).mjd
                            for item in a[nb, 3:5]
                        ]
                    )

                    self.date = [
                        pyDate.Date(year=item[0], doy=item[1]) for item in a[nb, 3:5]
                    ]
                    ...
                    raise pyETMException(
                        "No viable GAMIT solutions available for %s (all blunders!)\n"
                        "  -> min distance to station coordinate is %.1f meters"
                        % (stn_id, dd.min())
                    )
            else:
                raise pyETMException(
                    "No GAMIT polyhedrons vertices available for %s" % stn_id
                )
...
class ListSoln(GamitSoln):
    ...
    def __init__(
        ...
        NetworkCode,
        StationCode,
        stack_name="file-unknown",
        station_metadata=None,
    ):
        if not station_metadata:
            # instance can use the database to initialize the object
            logger.info(
                "Solution from file for station %s.%s" % (NetworkCode, StationCode)
            )
            super(ListSoln, self).__init__(
                cnn=cnn,
            ...
            logger.info(
                "Solution from file, station metadata for %s.%s from external source"
                % (NetworkCode, StationCode)
            )
            ...
            if self.solutions >= 1:
                        ...
                        np.sqrt(
                            np.sum(
                                np.square(
                                    a[:, 0:3]
                                    - np.array(
                                        [self.auto_x[0], self.auto_y[0], self.auto_z[0]]
                                    )
                                ),
                                axis=1,
                            )
                ...
                raise pyETMException(
                    "No GAMIT polyhedrons vertices available for %s" % StationCode
                )
...
class JumpTable:
    def __init__(
        ...
        self.earthquakes = Earthquakes(
            cnn, NetworkCode, StationCode, soln, t, FitEarthquakes, models
        )
        self.generic_jumps = GenericJumps(
            cnn, NetworkCode, StationCode, soln, t, FitGenericJumps
        )
        ...
        if len(self.table) > 0:
            ...
            if jump:
                dt = np.max(t[jump.design[:, -1] != 0]) - np.min(
                    t[jump.design[:, -1] != 0]
                )
                ...
                if jump.p.jump_type == CO_SEISMIC_JUMP_DECAY and (
                    ...
                    jump.param_count -= (
                        jump.nr
                    )  # subtract from param count the number of relaxations
        ...
        jcs = [
            ...
            for j in self.table
            if (
                j.p.jump_type == CO_SEISMIC_JUMP_DECAY
                or j.p.jump_type == CO_SEISMIC_DECAY
            )
            and j.fit is True
        ]
        if len(jcs) > 1:
            for j, i in zip(jcs[0:], jcs[1:]):
                ...
                j.constrain_data_points = np.count_nonzero(
                    t[np.logical_and(t > j.min_date, t < i.min_date)]
                )
            jcs[-1].constrain_years = t.max() - jcs[-1].min_date
            jcs[-1].constrain_data_points = np.count_nonzero(
                t[np.logical_and(t > jcs[-1].min_date, t < t.max())]
            )
        elif len(jcs) == 1:
            jcs[0].constrain_years = t.max() - jcs[0].min_date
            jcs[0].constrain_data_points = np.count_nonzero(
                t[np.logical_and(t > jcs[0].min_date, t < t.max())]
            )
        ...
        for ja in auto_jumps:
            ...
            for jt in self.table:
                ...
                if (
                    abs(jt.date.fyear - ja) < 2 / 365.25
                    or pyDate.Date(fyear=ja).mjd - 1 in soln.gaps
                ):
                    present = True
                    break
            if not present:
                ...
                logger.info(
                    prYellow(
                        "Possible unmodeled jump detected on %.3f (%s / %s)"
                        % (ja, date.datetime().strftime("%Y-%m-%d"), date.ddd())
                    )
                )
                self.auto_jumps.append(
                    Jump(
                        NetworkCode,
                        StationCode,
                        soln,
                ...
                if a.size:
                    if A.size:
                        ...
                        if np.linalg.cond(tA) < 1e10:
                            ...
                            jump.remove_from_fit()
                            warnings.warn(
                                "%s had to be removed due to high condition number"
                                % str(jump)
                            )
                    else:
                        A = a
        ...
        for jump in self.table:
            ...
            if jump.fit:
                ...
                for j, _ in enumerate(np.arange(jump.param_count)):
                    if (
                        j == 0
                        and jump.p.jump_type is not CO_SEISMIC_DECAY
                        and jump.action != "M"
                    ):
                        fmt_str = "{}            - {} {} {}"
                        ...
                        output_n.append(
                            fmt_str.format(
                                jump.date.yyyyddd(), m, jump.action, epi_dist
                            )
                        )
                        output_e.append(
                            fmt_str.format(
    ...
    def detect_jumps(self, soln):
        ...
        ecef = np.array(
            [soln.x - soln.auto_x, soln.y - soln.auto_y, soln.z - soln.auto_z]
        )
        ...
        jump_times = find_sets_with_tolerance(
            self.find_jumps(e_cluster_labels, soln.t)
...
class Jump(EtmFunction):
    ...
    def __init__(
        ...
        super(Jump, self).__init__(
            NetworkCode=NetworkCode, StationCode=StationCode, soln=soln
        )
        ...
        if (
            dtype not in (CO_SEISMIC_JUMP, CO_SEISMIC_DECAY, CO_SEISMIC_JUMP_DECAY)
            and not silent
        ):
            logger.info(
                "Mec jump -> Adding jump on %s type: %s; Action: %s; Fit: %s"
                % (
                    self.date.yyyyddd(),
                    type_dict[dtype],
                    action,
                    "T" if self.fit else "F",
                )
            )
        Jump.rehash(self)
    ...
    def __eq__(self, jump):
        if not isinstance(jump, Jump):
            raise pyETMException(
                "type: " + str(type(jump)) + " invalid. Can compare two Jump objects"
            )
    ...
    def __check_cmp(self, jump):
        if not isinstance(jump, Jump):
            raise pyETMException(
                "type: " + str(type(jump)) + " invalid.  Can only compare Jump objects"
            )
...
class CoSeisJump(Jump):
    def __init__(
        ...
        Jump.__init__(
            self, NetworkCode, StationCode, soln, t, date, metadata, dtype, action, fit
        )
        ...
        logger.info(
            "Geo jump -> Adding jump on %s type: %s; relax: %s; Mag: %.1f; Action: %s; Fit: %s"
            % (
                self.date.yyyyddd(),
                type_dict[dtype],
                str(relaxation),
        ...
        if postseismic is not None and self.p.jump_type in (
                ...
                if event.date == self.date and self.fit:
                    ...
                    for i, r in enumerate(event.relaxation):
                        if r in self.p.relaxation:
                            ...
                            self.p.relaxation = np.array(
                                [rr for rr in self.p.relaxation if rr != r]
                            )
                            self.nr = self.p.relaxation.shape[0]
                            # remove one from the parameter count
                            self.param_count -= 1
                            logger.info(
                                "Geophysical Jump -> Modifying %s: removing log decay with T=%.3f "
                            ...
                            if (
                                self.nr == 0
                                and self.p.jump_type == CO_SEISMIC_JUMP_DECAY
                            ):
    ...
    def rehash(self):
        ...
        self.p.hash = crc32(
            ...
            + str(self.p.jump_type)
            + str(self.p.relaxation)
            + str(self.fit)
            + VERSION
        )
    ...
    def eval(self, t):
        ...
        for i, T in enumerate(self.p.relaxation):
            hl[t > self.date.fyear, i] = np.log10(
                1.0 + (t[t > self.date.fyear] - self.date.fyear) / T
            )
        ...
        if np.any(hl):
            if self.p.jump_type == CO_SEISMIC_JUMP_DECAY:
                ...
                if cond_num > 5:
                    logger.info(
                        prYellow(
                            "CO_SEISMIC_JUMP_DECAY condition number: %.1f" % cond_num
                        )
                    )
                else:
                    logger.info(
...
class Earthquakes:
    def __init__(
        self, cnn, NetworkCode, StationCode, soln, t, FitEarthquakes=True, models=()
    ):
        self.StationCode = StationCode
        self.NetworkCode = NetworkCode
        ...
        if FitEarthquakes:
            ...
            jp = cnn.query_float(
                "SELECT * FROM etm_params WHERE \"NetworkCode\" = '%s' AND \"StationCode\" = '%s' "
                "AND soln = '%s' AND jump_type <> 0 AND object = 'jump'"
                % (
                    NetworkCode,
                    StationCode,
                    "gamit" if soln.type == "file" else soln.type,
                ),
                as_dict=True,
            )
            ...
            for mag, date, epi_lon, epi_lat, j_type, metadata in eq_jumps:
                ...
                jumps = [
                    (m, d, elon, elat, j_type, meta)
                    for m, d, elon, elat, j_type, meta in eq_jumps
                ...
                if len(jumps) > 1:
                    ...
                    for m, d, epi_lon, epi_lat, j_type, meta in jumps:
                        ...
                        table = {
                            j["action"]
                            for j in jp
                            if j["Year"] == d.year and j["DOY"] == d.doy
                        }
                    ...
                    table = {
                        j["action"]
                        for j in jp
                        if j["Year"] == date.year and j["DOY"] == date.doy
                    }
                    # get a different relaxation for this date
                    relax = [
                        j["relaxation"]
            ...
            notin = (
                ' AND ("Year", "DOY") NOT IN (' + ",".join(notin) + ")" if notin else ""
            )
            ...
            for j in jp:
                f_jumps.append(
                    CoSeisJump(
                        ...
                        np.array(j["relaxation"]),
                        "manual geophysical jump",
                        magnitude=0.0,
                        action="+",
                        models=models,
                        epi_distance=0.0,
    ...
    def __init__(self, cnn, NetworkCode, StationCode, soln, t, FitGenericJumps=True):
        ...
        try:
            ...
            for stninfo in self.stninfo.records[1:]:
                ...
                if (
                    prev_red["AntennaCode"] != stninfo["AntennaCode"]
                    or prev_red["RadomeCode"] != stninfo["RadomeCode"]
                    or action != "A"
                ):
                    # add to list only if:
                    ...
                    self.table.append(
                        Jump(
                            ...
                            "Antenna: %s->%s"
                            % (
                                prev_red["AntennaCode"] + " " + prev_red["RadomeCode"]
                                if prev_red["RadomeCode"]
                                else prev_red["AntennaCode"] + " NONE",
                                stninfo["AntennaCode"] + " " + stninfo["RadomeCode"]
                                if stninfo["RadomeCode"]
                                else stninfo["RadomeCode"] + " NONE",
                            ),
                            dtype=ANTENNA_CHANGE,
                            action=action,
                            fit=(
                                "+" in table
                                or (self.add_metadata_jumps and "-" not in table)
                            ),
                        )
                    )
                prev_red = stninfo
            ...
            logger.info(
                "No station information found for %s.%s, mechanical jumps may be incomplete"
                % (NetworkCode, StationCode)
            )
...
class Periodic(EtmFunction):
    ...
    def __init__(self, cnn, NetworkCode, StationCode, soln, t, FitPeriodic=True):
        ...
        if t.size > 1 and FitPeriodic:
            ...
            self.nyquist = ((1 / self.p.frequencies) / 2.0) * 0.5 * 1 / 365.25
        ...
        logger.info(
            "Periodic -> Frequency count: %i; FitPeriodic: %s"
            % (self.frequency_count, str(FitPeriodic))
        )
        ...
        self.format_str = (
            ...
            + " ("
            + ", ".join(
                "%.1f yr" % i for i in (1 / (self.p.frequencies * 365.25)).tolist()
            )
            + ") N: %s E: %s U: %s [mm]"
        )
    ...
    def get_design_ts(self, ts):
        ...
        if self.frequency_count > 0:
            ...
            As = np.array(
                sin(2 * pi * f * 365.25 * np.tile(ts[:, np.newaxis], (1, f.shape[1])))
            )
            Ac = np.array(
                cos(2 * pi * f * 365.25 * np.tile(ts[:, np.newaxis], (1, f.shape[1])))
            )
    ...
    def print_parameters(self):
        ...
        return self.format_str % (
            np.array_str(an * 1000.0, precision=1),
...
class Polynomial(EtmFunction):
    ...
    def __init__(self, cnn, NetworkCode, StationCode, soln, t, t_ref=0, models=()):
        ...
        if len(model) > 0 and not model[0].fit:
            logger.info(
                "Polynomial -> Interseismic velocity provided: removing velocity from fit"
            )
            ...
            try:
                ...
                if etm_param["Year"] and etm_param["DOY"]:
                    self.p.t_ref = pyDate.Date(
                        year=etm_param["Year"], doy=etm_param["DOY"]
                    ).fyear
                    t_ref = self.p.t_ref
            ...
            logger.info(
                "Polynomial -> Fitting %i term(s), conventional epoch %.3f"
                % (self.terms, self.p.t_ref)
            )
    ...
    def print_parameters(self, ref_xyz, lat, lon):
        ...
        for p in np.arange(self.terms):
            if p == 0:
                ...
                params = np.append(
                    params,
                    (n * 1000, sn * 1000, e * 1000, se * 1000, u * 1000, su * 1000),
                )
...
class Design(np.ndarray):
    def __new__(
        ...
        shape = (
            Linear.design.shape[0],
            Linear.param_count + Jumps.param_count() + Periodic.param_count,
        )
        A = super(Design, subtype).__new__(
    ...
    def __call__(self, ts=None, constrains=False):
        if ts is None:
            if constrains and self.constrains.size:
                ...
                A.resize(
                    (self.shape[0] + self.constrains.shape[0], self.shape[1]),
                    refcheck=False,
                )
                # apply constrains
    ...
    def get_p(self, constrains=False):
        ...
        return np.ones(
            self.shape[0]
            if not constrains
            else (self.shape[0] + self.constrains.shape[0])
        )
...
class ETM:
    def __init__(
        ...
        logger.info(
            "First obs %.3f last obs %.3f nobs %i"
            % (np.min(soln.t), np.max(soln.t), soln.t.size)
        )
        ...
        self.Periodic = Periodic(
            cnn, soln.NetworkCode, soln.StationCode, self.soln, self.soln.t, FitPeriodic
        )
        self.Jumps = JumpTable(
            cnn,
            soln.NetworkCode,
            soln.StationCode,
            self.soln,
        ...
        self.l = self.rotate_2neu(
            np.array(
                [
                    self.soln.x - self.soln.auto_x,
        ...
        if len(models) > 0:
            ...
            self.L = self.rotate_2xyz(self.l) + np.array(
                [self.soln.auto_x, self.soln.auto_y, self.soln.auto_z]
            )
    ...
    def display_postseismic_params(self, postseismic):
        for event in postseismic:
            ...
            for j in self.Jumps.table:
                ...
                if j.date == event["date"] and j.p.jump_type in (
                    CO_SEISMIC_JUMP_DECAY,
                    CO_SEISMIC_DECAY,
                    CO_SEISMIC_JUMP,
                ):
                    ...
                    jump.action = "M"
                    if not jump.fit and j.p.jump_type in (
                        CO_SEISMIC_JUMP_DECAY,
                        CO_SEISMIC_JUMP,
                    ):
            ...
            if jump is None:
                ...
                jump = CoSeisJump(
                    ...
                    self.soln.t,
                    event["date"],
                    np.array(event["relaxation"]),
                    "mag=??",
                    CO_SEISMIC_JUMP_DECAY
                    if self.soln.t.min() < event["date"].fyear
                    else CO_SEISMIC_DECAY,
                    0.0,
                    "M",
                    fit=True,
    ...
    def apply_postseismic_model(self, postseismic):
        ...
        for event in postseismic:
            ...
            for i, r in enumerate(event["relaxation"]):
                ...
                hl = np.zeros((t.shape[0],))
                pmodel = np.zeros((3, t.shape[0]))
                hl[t > date.fyear] = np.log10(
                    1.0 + (t[t > date.fyear] - date.fyear) / r
                )
                # apply the amplitudes
                for j in range(3):
    ...
    def run_adjustment(self, cnn, l, soln, ignore_db_params=False):
        if self.A is not None:
            ...
            etm_objects = cnn.query_float(
                "SELECT * FROM etms WHERE \"NetworkCode\" = '%s' "
                "AND \"StationCode\" = '%s' AND soln = '%s' AND stack = '%s'"
                % (
                    self.NetworkCode,
                    self.StationCode,
                    self.soln.type,
                    self.soln.stack_name,
                ),
                as_dict=True,
            )
            ...
            ob_hash_sum = (
                self.Periodic.p.hash + self.Linear.p.hash + self.hash + jumps_hash
            )
            cn_object_sum = len([o.p.hash for o in self.Jumps.table if o.fit]) + 2

            # -1 to account for the var_factor entry
            if (
                len(etm_objects) - 1 == cn_object_sum
                and db_hash_sum == ob_hash_sum
                and not ignore_db_params
            ):
                logger.info(
                    "ETM -> Loading parameters from database (db hash %i; ob hash %i)"
                ...
                logger.info(
                    "ETM -> Estimating parameters (db hash %i; ob hash %i)"
                    % (db_hash_sum, ob_hash_sum)
                )
                ...
                if len(self.models) == 0:
                    cnn.query(
                        "DELETE FROM etms WHERE \"NetworkCode\" = '%s' AND "
                ...
                if self.soln.type == "dra":
                    ...
                    cnn.query(
                        "DELETE FROM gamit_soln_excl WHERE \"NetworkCode\" = '%s' AND "
                ...
                while j < 10:
                    ...
                    factor = []
                    for i in range(3):
                        x, sigma, index, residuals, fact, w, cova = self.adjust_lsq(
                            self.A, l[i]
                        )
                    ...
                    for jump in self.Jumps.table:
                        if (
                            jump.fit
                            and jump.p.jump_type
                            in (CO_SEISMIC_JUMP_DECAY, CO_SEISMIC_DECAY)
                            and np.any(np.abs(jump.p.params[:, -jump.nr :]) > 4)
                        ):
                            # unrealistic, remove
                            jump.remove_from_fit()
                            do_again = True
                            logger.info(
                                "ETM -> Unrealistic jump detected (%s : %s) on %s, removing and redoing fit"
                                % (
                                    np.array_str(
                                        jump.p.params[:, -jump.nr :].flatten(),
                                        precision=1,
                                    ),
    ...
    def process_covariance(self):
        ...
        # load the covariances using the correlations
        cov[0] = (
            np.corrcoef(self.R[0][f], self.R[1][f])[0, 1]
            * self.factor[0]
            * self.factor[1]
        )
        cov[1] = (
            np.corrcoef(self.R[1][f], self.R[2][f])[0, 1]
            * self.factor[1]
            * self.factor[2]
        )
        cov[2] = (
            np.corrcoef(self.R[0][f], self.R[2][f])[0, 1]
            * self.factor[0]
            * self.factor[2]
        )
    ...
    def save_excluded_soln(self, cnn):
        ...
        if self.F.size > 0:
            for date, f, r in zip(
                ...
                if (
                    not cnn.query_float(
                        "SELECT * FROM gamit_soln_excl WHERE \"NetworkCode\" = '%s' AND "
                    ...
                    cnn.query(
                        'INSERT INTO gamit_soln_excl ("NetworkCode", "StationCode", "Project", "Year", "DOY", '
    ...
    def save_parameters(self, cnn):
        ...
        if self.param_origin == ESTIMATION:
            ...
            cnn.query(
                'INSERT INTO etms ("NetworkCode", "StationCode", soln, object, params, hash, stack) VALUES '
                "('%s', '%s', '%s', 'var_factor', '%s', %i, '%s')"
                % (
                    self.NetworkCode,
                    self.StationCode,
                    self.soln.type,
                    to_postgres(self.factor),
                    self.hash,
                    self.soln.stack_name,
                )
            )
        ...
        if self.A is not None:
            ...
            if self.plot_jumps_removed:
                for j in [
                    j
                    for j in self.Jumps.table
                    if j.p.jump_type is not CO_SEISMIC_DECAY and j.fit
                ]:
                    if j.p.jump_type is CO_SEISMIC_JUMP_DECAY:
                        ...
                        mj = mj + np.array(
                            [
                                (np.dot(a[:, 0], j.p.params[i, 0])) * 1000
                                for i in range(3)
                            ]
                        )
                        ...
                        mj = mj + np.array(
                            [
                                (np.dot(j.eval(self.soln.ts), j.p.params[i])) * 1000
                                for i in range(3)
                            ]
                        )
                        oj = oj + np.array(
                            [
                                (np.dot(j.eval(self.soln.t), j.p.params[i])) * 1000
                                for i in range(3)
                            ]
                        )
            ...
            if self.plot_polynomial_removed:
                mp = [
                    (
                        np.dot(
                            self.Linear.get_design_ts(self.soln.ts),
                            self.Linear.p.params[i],
                        )
                    )
                    * 1000
                    for i in range(3)
                ...
                op = [
                    (
                        np.dot(
                            self.Linear.get_design_ts(self.soln.t),
                            self.Linear.p.params[i],
                        )
                    )
                    * 1000
            ...
            m = [(np.dot(self.As, self.C[i])) * 1000 - mj[i] - mp[i] for i in range(3)]
        else:
        ...
        if self.A is not None:
            ...
            f.suptitle(
                ...
                % (
                    ...
                    self.soln.lat,
                    self.soln.lon,
                    self.Linear.print_parameters(
                        self.ce_pos, self.soln.lat, self.soln.lon
                    ),
                    ...
                    ""
                    if not self.plot_polynomial_removed
                    else LABEL("polynomial removed"),
                ),
                fontsize=9,
                family="monospace",
            )
            ...
            for i, ax in enumerate(axis_vect):
                ...
                if not residuals:
                    ...
                    ax.plot(
                        self.soln.ts,
                        -np.repeat(fneu[i], self.soln.ts.shape[0]) * LIMIT,
                        "b",
                        alpha=0.1,
                    )
                    ax.plot(
            ...
            f.suptitle(
                LABEL("station")
                + " %s (%s %.2f%%) lat: %.5f lon: %.5f"
                % (
                    stationID(self),
                    self.soln.type.upper(),
                    self.soln.completion,
                    self.soln.lat,
                    self.soln.lon,
                )
    ...
    def onpick(self, event):
        ...
        jtype = int(
            eval(input(" -- Enter type of jump (0 = mechanic; 1 = geophysical): "))
        )
        if jtype == 1:
        ...
        if self.A is not None:
            ...
            f.suptitle(
                ...
                % (
                    ...
                    " ".join(
                        "%10.3e" % i
                        for i in [self.covar[0, 1], self.covar[0, 2], self.covar[1, 2]]
                    ),
                ),
                fontsize=9,
                family="monospace",
            ...
            ellipse = Ellipse(
                (np.mean(self.R[1][filt]), np.mean(self.R[1][filt])),
                width=2.0 * np.sqrt(w[0]) * 2.5 * 1000,
                height=2.0 * np.sqrt(w[1]) * 2.5 * 1000,
                angle=theta,
                facecolor="none",
                edgecolor="red",
                zorder=3,
            ...
            ax.set_title(
                "%s %s-%s" % (LABEL("residual plot"), LABEL("north"), LABEL("east"))
            )
            ax.axis("equal")
            f.canvas.draw()
            ax.legend()
            ...
            n, bins, patches = ax.hist(
                N, 200, alpha=0.75, facecolor="blue", orientation="horizontal"
            )
    ...
    def set_lims(self, t_win, plt, ax):
        if t_win is None:
            ...
            if t_win[0] == t_win[1]:
                t_win[0] = t_win[0] - 1.0 / 365.25
                t_win[1] = t_win[1] + 1.0 / 365.25
    ...
    def todictionary(self, time_series=False, model=False):
        ...
        if time_series:
            etm["time_series"] = {
                "t": np.array([self.soln.t.tolist(), self.soln.mjd.tolist()])
                .transpose()
                .tolist(),
                ...
                "model_neu": []
                if self.A is None or not model
                else [(np.dot(self.As, self.C[i]).tolist()) for i in range(3)],
                "filter": []
                if self.A is None
                else np.logical_and(
                    np.logical_and(self.F[0], self.F[1]), self.F[2]
                ).tolist(),
            }
    ...
    def get_xyz_s(
        ...
        doy,
        jmp=None,
        sigma_h=SIGMA_FLOOR_H,
        sigma_v=SIGMA_FLOOR_V,
        force_model=False,
    ):
        ...
        for jump in self.Jumps.table:
            if (
                ...
                and jump.fit
                and np.sqrt(np.sum(np.square(jump.p.params[:, 0]))) > 0.02
            ):
                window = jump.date
                # if no pre or post specified, then determine using the time of the jump
                if jmp is None:
                    if (
                        jump.date.datetime().hour + jump.date.datetime().minute / 60.0
                    ) < 12:
                        jmp = "post"
                    else:
                        jmp = "pre"
        ...
        if self.A is not None:
            ...
            if (
                np.sqrt(
                    np.square(self.Linear.p.params[0, 1])
                    + np.square(self.Linear.p.params[1, 1])
                    + np.square(self.Linear.p.params[2, 1])
                )
                > 0.2
            ):
        ...
        # apply floor sigmas
        sig = np.sqrt(
            np.square(sig) + np.square(np.array([[sigma_h], [sigma_h], [sigma_v]]))
        )
    ...
    def rotate_sig_cov(self, sigmas=None, covar=None):
        if sigmas is None and covar is None:
            raise pyETMException(
                "Error in rotate_sig_cov: must provide either sigmas or covariance matrix"
            )
    ...
    def get_outliers_list(self):
        ...
        filt = self.F[0] * self.F[1] * self.F[2]
        return [
            (self.NetworkCode, self.StationCode, pyDate.Date(mjd=mjd))
    ...
    def pull_params(self):
        ...
        poly_dict = {"terms": self.Linear.terms, "Year": date.year, "DOY": date.doy}
        ...
        jumps = [
            {
                ...
                "relaxation": []
                if jump.p.jump_type < CO_SEISMIC_JUMP_DECAY
                else jump.p.relaxation.tolist(),
    ...
    def push_params(
        ...
        if reset_polynomial:
            ...
            cnn.delete(
                ...
                soln="gamit" if self.soln.type == "file" else self.soln.type,
                object="polynomial",
            )
...
class GamitETM(ETM):
    def __init__(
        ...
        if gamit_soln is None:
            ...
            self.gamit_soln = GamitSoln(
                cnn, self.polyhedrons, NetworkCode, StationCode, stack_name
            )
    ...
    def get_etm_soln_list(self, use_ppp_model=False, cnn=None):
        ...
        if self.A is None:
            ...
            neu = [np.dot(self.A, self.C[i]) for i in range(3)]
        else:
            ...
            if etm.A is None:
                ...
                neu = [np.dot(etm.As[index, :], etm.C[i]) for i in range(3)]

                del etm

        rxyz = self.rotate_2xyz(np.array(neu)) + np.array(
            [self.soln.auto_x, self.soln.auto_y, self.soln.auto_z]
        )

        return [
            (stn_id, x, y, z, date.year, date.doy, date.fyear)
            for x, y, z, date in zip(rxyz[0], rxyz[1], rxyz[2], self.gamit_soln.date)
        ]
...
class DailyRep(ETM):
    def __init__(
        ...
        ETM.__init__(
            ...
            self.soln,
            no_model,
            FitEarthquakes=False,
            FitGenericJumps=False,
            FitPeriodic=False,
            plotit=plotit,
        )
    ...
    def get_residuals_dict(self):
        ...
        xyz = self.rotate_2xyz(np.array(neu)) + np.array(
            [self.soln.auto_x, self.soln.auto_y, self.soln.auto_z]
        )
        ...
        return [
            ...
            for x, y, z, sigx, sigy, sigz, date in zip(
                rxyz[0], rxyz[1], rxyz[2], px, py, pz, self.soln.date
            )
        ]
