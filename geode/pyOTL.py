...
# app
from . import pyEvents, pyRunWithRetry
...
class OceanLoading:
    def __init__(self, StationCode, grdtab, otlgrid, x=None, y=None, z=None):
        ...
        if not os.path.isfile(grdtab):
            raise pyOTLException(
                "grdtab could not be found at the specified location: " + grdtab
            )
        else:
            self.grdtab = grdtab
    ...
    def calculate_otl_coeff(self, x=None, y=None, z=None):
        if not self.x and (x is None or y is None or z is None):
            raise pyOTLException(
                "Cartesian coordinates not initialized and not provided in calculate_otl_coef"
            )
