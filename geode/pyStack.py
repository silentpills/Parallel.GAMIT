...
# app
from . import dbConnection, pyETM
from .pyDate import Date
from .pyETM import pi
from .Utils import ct2lg, file_write, json_converter, lg2ct, process_stnlist, stationID
...
def print_residuals(
    NetworkCode, StationCode, residuals, lat, lon, components=("N", "E", "U")
):
    ...
    for i, c in enumerate(components):
        r = (
            r
            + "    %s: " % c
            + " ".join("%8.4f" % np.multiply(k, 1000) for k in residuals[i])
            + " %s: " % ccomponent[i]
            + " ".join("%8.4f" % np.multiply(k, 1000) for k in cresiduals[i])
            + "\n"
        )
...
class Stack(list):
    def __init__(self, cnn, project, name, redo=False, end_date=None):
        ...
        if redo:
            ...
            for d in tqdm(
                self.dates, ncols=160, desc=" >> Initializing the stack polyhedrons"
            ):
            ...
            print(
                " >> Determining differences between stack %s and GAMIT solutions for project %s..."
                % (name, project)
            )

            # build a dates vector
            dates = self.cnn.query_float(
                'SELECT "Year", "DOY" FROM stacks WHERE "name" = \'%s\' '
                'AND ("Year", "DOY") <= (%i, %i) '
                "UNION "
                'SELECT "Year", "DOY" FROM gamit_soln WHERE "Project" = \'%s\' '
                'AND ("Year", "DOY") <= (%i, %i) '
            ...
            for d in tqdm(
                self.dates,
                ncols=160,
                desc=" >> Checking station count for each day",
                disable=None,
            ):
                try:
                    s = self.cnn.query_float(
                        'SELECT count("X") FROM stacks WHERE "Project" = \'%s\' '
                        'AND "name" = \'%s\' AND "Year" = %i AND "DOY" = %i'
                        % (project, name, d.year, d.doy)
                    )
                    g = self.cnn.query_float(
                        'SELECT count("X") FROM gamit_soln WHERE "Project" = \'%s\' '
                        'AND "Year" = %i AND "DOY" = %i' % (project, d.year, d.doy)
                    )
                    if s[0] < g[0]:
                        self.cnn.query(
                            "DELETE FROM stacks WHERE \"Project\" = '%s' "
                            'AND "name" = \'%s\' AND "Year" = %i AND "DOY" = %i'
                            % (project, name, d.year, d.doy)
                        )
                    ...
                    tqdm.write(
                        " -- Error while processing %s: %s" % (d.yyyyddd(), str(e))
                    )
            ...
            for d in tqdm(
                ...
                try:
                    ...
                    self.append(
                        Polyhedron(self.stack_vertices, project, d, aligned=True)
                    )
                    ...
                    self.append(
                        Polyhedron(self.gamit_vertices, project, d, aligned=False)
                    )
    ...
    def calculate_etms(self):
        ...
        for s in tqdm(self.stations, ncols=160, desc=self.name, disable=None):
            ...
            try:
                ...
                ts = pyETM.GamitSoln(
                    self.cnn, ts, s["NetworkCode"], s["StationCode"], self.project
                )
    ...
    def remove_common_modes(self, target_periods=None):
        if target_periods is None:
            ...
            etm_objects = self.cnn.query_float(
                'SELECT etms."NetworkCode", etms."StationCode", stations.lat, '
                "stations.lon, "
                ...
                "AND frequencies <> '{}' "
                'ORDER BY etms."NetworkCode", etms."StationCode"' % self.name,
                as_dict=True,
            )
        else:
            use_stations = []
            for s in target_periods.keys():
                ...
                if not np.isnan(target_periods[s]["365.250"]["n"][0]) and not np.isnan(
                    target_periods[s]["182.625"]["n"][0]
                ):
            ...
            etm_objects = self.cnn.query_float(
                ...
                % (self.name, "', '".join(use_stations)),
                as_dict=True,
            )

        # load the frequencies to subtract
        frequencies = self.cnn.query_float(
            "SELECT frequencies FROM etms WHERE soln = 'gamit' AND "
            "object = 'periodic' AND frequencies <> '{}' AND stack = '%s' "
            "GROUP BY frequencies" % self.name,
            as_dict=True,
        )

        if len(frequencies) == 0:
            tqdm.write(
                " -- No periodic components available! Maybe time series are not long enough "
        ...
        tqdm.write(
            " -- Reporting periodic residuals (in mm) before %s"
        ...
        for s, p in enumerate(etm_objects):
            ...
            q = self.cnn.query_float(
                "SELECT frequencies as freq, * FROM etms "
            ...
            print_residuals(
                p["NetworkCode"], p["StationCode"], params, p["lat"], p["lon"]
            )

            # convert from NEU to XYZ
            for j in range(params.shape[1]):
                params[:, j] = np.array(
                    lg2ct(params[0, j], params[1, j], params[2, j], p["lat"], p["lon"])
                ).flatten()
        ...
        x = self.cnn.query_float(
            "SELECT 0, -auto_z*1e-9, auto_y*1e-9, 1, 0, 0, auto_x*1e-9 FROM stations WHERE "
            '"NetworkCode" || \'.\' || "StationCode" '
            'IN (%s) ORDER BY "NetworkCode", "StationCode"' % sql_where
        )

        y = self.cnn.query_float(
            "SELECT auto_z*1e-9, 0, -auto_x*1e-9, 0, 1, 0, auto_y*1e-9 FROM stations WHERE "
            '"NetworkCode" || \'.\' || "StationCode" '
            'IN (%s) ORDER BY "NetworkCode", "StationCode"' % sql_where
        )

        z = self.cnn.query_float(
            "SELECT -auto_y*1e-9, auto_x*1e-9, 0, 0, 0, 1, auto_z*1e-9 FROM stations WHERE "
            '"NetworkCode" || \'.\' || "StationCode" '
            'IN (%s) ORDER BY "NetworkCode", "StationCode"' % sql_where
        )
        Ax = np.array(x)
        ...
        for freq in f_vector:
            for i, cs in enumerate((np.sin, np.cos)):
                L = np.row_stack(
                    (
                        ox[f_vector == freq, :, i].flatten(),
                        oy[f_vector == freq, :, i].flatten(),
                        oz[f_vector == freq, :, i].flatten(),
                    )
                ).flatten()
                ...
                tqdm.write(
                    " -- Transformation for %s(2 * pi * 1/%.2f) : %s"
                    % (
                        cs.__name__,
                        np.divide(1.0, freq),
                    ...
                    % " ".join(
                        "%s" % ss for ss in np.array(xyzstn)[np.logical_not(index)]
                    )
                )
                ...
                solution_vector.append(
                    [
                        "%s(2 * pi * 1/%.2f)" % (cs.__name__, np.divide(1.0, freq)),
                        c.tolist(),
                    ]
                )
                ...
                for poly in tqdm(
                    self,
                    ncols=160,
                    desc=" -- Applying transformation -> %s(2 * pi * 1/%.2f)"
                    % (cs.__name__, np.divide(1.0, freq)),
                    disable=None,
                ):
                    # subtract the inverted common modes
                    poly.vertices["x"] -= cs(
                        2 * pi * freq * 365.25 * poly.date.fyear
        ...
        tqdm.write(
            " -- Reporting periodic residuals (in mm) after %s\n"
        ...
        for s, p in enumerate(etm_objects):
            ...
            self.cnn.query(
                "DELETE FROM etms WHERE \"soln\" = 'gamit' AND \"NetworkCode\" = '%s' AND "
                "\"StationCode\" = '%s' AND stack = '%s'"
                % (p["NetworkCode"], p["StationCode"], self.name)
            )
            # save the time series
            ts = pyETM.GamitSoln(
                self.cnn, stn_ts, p["NetworkCode"], p["StationCode"], self.name
            )
            # create the ETM object
            pyETM.GamitETM(
                self.cnn, p["NetworkCode"], p["StationCode"], False, False, ts
            )
            ...
            print_residuals(
                p["NetworkCode"], p["StationCode"], residuals, p["lat"], p["lon"]
            )

            # convert from NEU to XYZ
            for j in range(residuals.shape[1]):
                residuals[:, j] = np.array(
                    lg2ct(
                        residuals[0, j],
                        residuals[1, j],
                        residuals[2, j],
            ...
            for i, f in enumerate(p["freq"]):
                rx[f_vector == f, s] = residuals[
                    0, i : i + param_count + 1 : param_count
                ]
                ry[f_vector == f, s] = residuals[
                    1, i : i + param_count + 1 : param_count
                ]
                rz[f_vector == f, s] = residuals[
                    2, i : i + param_count + 1 : param_count
                ]

        # save the position space residuals
        self.periodic_space = {
            "stations": {
                "codes": [stationID(p) for p in etm_objects],
    ...
    def align_spaces(self, target_dict):
        ...
        for stn in use_stations:
            if not np.isnan(target_dict[stn]["x"]):
                ...
                try:
                    ts = pyETM.GamitSoln(
                        self.cnn, self.get_station(net, ssn), net, ssn, self.name
                    )
                    ...
                    tqdm.write(
                        " -- Station %s in the constraints file but no vertices in polyhedron: %s"
                        % (stn, str(e))
                    )
                    continue
        ...
        tqdm.write(
            " -- Reporting position space residuals (in mm) before and after frame alignment\n"
        ...
        for i, stn in enumerate(a_stn):
            ...
            lla = self.cnn.query_float(
                "SELECT lat, lon FROM stations WHERE \"NetworkCode\" = '%s' "
                "AND \"StationCode\" = '%s'" % (n, s)
            )[0]
        ...
        # save the position space residuals
        self.position_space = {
            "stations": {"codes": a_stn.tolist(), "latlon": stn_lla},
        ...
        for poly in tqdm(
            self,
            ncols=160,
            desc=" -- Applying position space transformation",
            disable=None,
        ):
        ...
        for s, p in enumerate(etm_objects):
            ...
            v = np.array(
                lg2ct(params[0, 1], params[1, 1], params[2, 1], p["lat"], p["lon"])
            ).flatten()
        ...
        for poly in tqdm(
            self,
            ncols=160,
            desc=" -- Applying velocity space transformation",
            disable=None,
        ):
        ...
        tqdm.write(
            " -- Reporting velocity space residuals (in mm/yr) before and after frame alignment\n"
            "         Before   After |     Before   After  "
        )
        ...
        for s, p in enumerate(etm_objects):
            ...
            lla = self.cnn.query_float(
                "SELECT lat, lon FROM stations WHERE \"NetworkCode\" = '%s' "
                "AND \"StationCode\" = '%s'" % (p["NetworkCode"], p["StationCode"])
            )[0]

            print_residuals(
                p["NetworkCode"],
                p["StationCode"],
                np.array([[dvx[s], dvxa[s]], [dvy[s], dvya[s]], [dvz[s], dvza[s]]]),
                lla[0],
                lla[1],
                ["X", "Y", "Z"],
            )
        ...
        self.velocity_space = {
            ...
            "residuals_before_alignment": np.column_stack(
                (dvx.flatten(), dvy.flatten(), dvz.flatten())
            ).tolist(),
            "residuals_after_alignment": np.column_stack(
                (dvxa.flatten(), dvya.flatten(), dvza.flatten())
            ).tolist(),
            "reference_date": ref_date.fyear,
    ...
    def build_design(self, stations, scale=False):
        ...
        x = self.cnn.query_float(
            "SELECT 0, -auto_z*1e-9, auto_y*1e-9, 1, 0, 0%s FROM stations WHERE "
            '"NetworkCode" || \'.\' || "StationCode" '
        ...
        y = self.cnn.query_float(
            "SELECT auto_z*1e-9, 0, -auto_x*1e-9, 0, 1, 0%s FROM stations WHERE "
            '"NetworkCode" || \'.\' || "StationCode" '
        ...
        z = self.cnn.query_float(
            "SELECT -auto_y*1e-9, auto_x*1e-9, 0, 0, 0, 1%s FROM stations WHERE "
            '"NetworkCode" || \'.\' || "StationCode" '
...
class Polyhedron:
    def __init__(self, vertices, project, date, rot=True, aligned=False):
        ...
        self.vertices = vertices[
            np.logical_and(vertices["yr"] == date.year, vertices["dd"] == date.doy)
        ]
        ...
        rx = (
            np.array(
                [np.zeros(self.rows), -self.vertices["z"], self.vertices["y"]]
            ).transpose()
            * 1e-9
        )
        ry = (
            np.array(
                [self.vertices["z"], np.zeros(self.rows), -self.vertices["x"]]
            ).transpose()
            * 1e-9
        )
        rz = (
            np.array(
                [-self.vertices["y"], self.vertices["x"], np.zeros(self.rows)]
            ).transpose()
            * 1e-9
        )

        tx = np.array(
            [np.ones(self.rows), np.zeros(self.rows), np.zeros(self.rows)]
        ).transpose()
        ty = np.array(
            [np.zeros(self.rows), np.ones(self.rows), np.zeros(self.rows)]
        ).transpose()
        tz = np.array(
            [np.zeros(self.rows), np.zeros(self.rows), np.ones(self.rows)]
        ).transpose()
    ...
    def ax(self, scale=False):
        ...
        if scale:
            return np.concatenate(
                (self.Ax, self.vertices["x"][np.newaxis].transpose() * 1e-9), axis=1
            )
        else:
            return self.Ax
    ...
    def ay(self, scale=False):
        ...
        if scale:
            return np.concatenate(
                (self.Ay, self.vertices["y"][np.newaxis].transpose() * 1e-9), axis=1
            )
        else:
            return self.Ay
    ...
    def az(self, scale=False):
        ...
        if scale:
            return np.concatenate(
                (self.Az, self.vertices["z"][np.newaxis].transpose() * 1e-9), axis=1
            )
        else:
            return self.Az

    def align(
        self, target=None, set_aligned=True, helmert=None, scale=False, verbose=False
    ):
        """
        ...
        if target is not None:
            ...
            if verbose:
                tqdm.write(
                    " -- T: %s iterations: %i wrms: %.1f stations used: %i\n"
                    "    Down-weighted station components: %s"
                    % (
            ...
            if verbose:
                tqdm.write(
                    " -- T: %s -> externally provided"
                    % (" ".join("%7.4f" % cc for cc in helmert))
                )
        ...
        x = (
            np.dot(
                np.concatenate(
                    (self.ax(scale), self.ay(scale), self.az(scale)), axis=0
                ),
                c,
            )
            .reshape((3, self.rows))
