...
def cal_eig(matrix):
    ...
    )  # I convert matrix to complex values so that numba can manage this function
    ind = np.argsort(
        -(eig_val.real)
    )  # numpy.sort does not have descending mode (only ascending),so i make input negative and sort it
    eig_val = eig_val[ind]
    eig_vec = eig_vec[:, ind]
...
def inner_core_sol_0(core_data):
    ...
    k1 = np.sqrt(
        kk1
    )  # kk1 is always positive. no need to consider issue of complex value
    x = k1 * r
    ...
    output[1] = (
        r
        * (
            -sigma * f1 * kk1 * r**2 * jn[n]
            + 2
            * miu
            * (n * (n - 1) * h1 * jn[n] + (2 * f1 + n * (n + 1)) * k1 * r * jn[n + 1])
        )
        / r**2
    )
...
def inner_core_sol_complex(core_data, n):
    ...
    y[1, 1] = (
        -sigma * f2 * kk2 * r**2 * jn[n]
        + 2
        * miu
        * (n * (n - 1) * h2 * jn[n] + (2 * f2 + n * (n + 1)) * k2 * r * jn[n + 1])
    ) / r**2
    y[3, 1] = (
        miu
        * (
    ...
    output[0] = (
        y[4, 0] * c[0] + y[4, 1] * c[1] + y[4, 2]
    )  # phi = y[4,0] * c[0] + y[4,1] * c[1] + y[4,2]
    output[1] = (
        y_array[0] * c[0] + y_array[1] * c[1] + y_array[2]
    )  # Y = y1 * c[0] + y2 * c[1] + y3
    return output
...
def dvp_propagator(matrix_s2, matrix_s1):
    ...
    matrix_prop[:m, :m] = matrix_s1[:m, :m] @ (
        matrix_s2[:m, :m] + tmp1 @ matrix_s1[m:, :m] @ matrix_s2[:m, :m]
    )
    matrix_prop[:m, m:] = (
        matrix_s1[:m, :m] @ tmp1 @ matrix_s1[m:, m:] + matrix_s1[:m, m:]
    )
    matrix_prop[m:, :m] = (
        matrix_s2[m:, :m] + tmp2 @ matrix_s1[m:, :m] @ matrix_s2[:m, :m]
    )
...
def diff_eq2_S(mantle_data, layer_id, s1, s0, CONST_C):
    ...
    eq[0, 1] = 1 / c33
    eq[1, 0] = (
        -4 * CONST_C * rho_s * sbar * g_s + 2 * (c33 * (c11 + c12) - 2 * c13**2) / c33
    )
...
def diff_eq6(mantle_data, layer_id, s1, s0, CONST_B, CONST_C, m_nH):
    ...
    eq[3, 1] = (
        CONST_C * NN * rho_s * sbar * g_s - NN * (c33 * (c11 + c12) - 2 * c13**2) / c33
    ) / m_nH**3
    ...
    eq[4, 0] = (
        CONST_C * rho_s * sbar * g_s + (2 * c13**2 - c33 * (c11 + c12)) / c33
    ) / m_nH
...
@njit
def DVP_T_cmd2surface(
    mantle_data, depth_s, depth_f, m_source_layer, m_field_layer, m_nH
):
    rs = (CONST_R - depth_s) / CONST_R
    rf = (CONST_R - depth_f) / CONST_R
    if rs <= rf:
...
@njit
def DVP_S_core_mantle_boundary(
    core_data, ppgC, m_nH, CONST_B, CONST_C, CONST_RHO, CONST_GR
):
    if m_nH < 100:
        y = np.zeros((2,), dtype=np.complex128)
        tt = inner_core_sol_complex(core_data, m_nH)  # is complex value
        y[1] = (tt[1] - ppgC[1, 0] * tt[0] * m_nH) / ppgC[
            1, 1
        ]  # note in matlab the operator is \ instead of /
...
def DVP_S_dislocation(
    ...
    matrix_dvp_1, matrix_dvp_2, matrix_dvp_3, ID_SF = DVP_S_cmd2surface(
        mantle_data,
        diff_eq6,
        depth_s,
        depth_f,
    ...
    if m_nH == 1:
        ...
        if ID_SF == 0:  # I skipped inter because they are not used in the following.
            ...
            Trs = Trs - Step[3:, 1:]
            Urf = np.linalg.solve(
                matrix_dvp_2[:3, :3], (Urs - matrix_dvp_2[:3, 3:] @ Trs)
            )
        Hs[1:] = Urf[0, :].real
        Ls[1:] = Urf[1, :].real
        Ks[1:] = -Urf[2, :].real
        ...
        if ID_SF == 0:
            if (depth_f == 0) and (depth_s == 0):
                ...
                Cx1, dx1 = matrix_cd_internal0(
                    matrix_dvp_2, matrix_dvp_3, dx0[6:9, :] + Step[3:, :]
                )
...
def DVP_S0_core(core_data):
    ...
    for i in range(core_data.shape[0] - 1):
        ...
        ppgC = np.linalg.solve(
            B, ppgC
        )  # solve is faster and more accurate than np.linalg.inv(B) @ ppgC
...
def DVP_S0_dislocation(
    ...
    if ID_SF == 0:
        ...
        Tc = x * tM[2:]
        Tf = np.linalg.solve(
            matrix_dvp_1[:2, :2], (Tc - matrix_dvp_3[2:, :2] @ Uc)
        )  # matrix_dvp_1[1,1] / (Tc - matrix_dvp_3[2:,:2] @ Uc) may be wrong because the left should be a matrix instead of a value
...
def find_degree1_Tmode(mantle_data, depth_s, depth_f, m_source_layer):
    ...
    """
    x1 = (
        mantle_data[1 : m_source_layer + 2, 1]
        * (2 / 3)
        * (s32 - s31)
        / (s22 - s21)
        * (s42 - s41)
    ...
    """
    x2 = (
        mantle_data[m_source_layer + 1 :, 1]
        * (2 / 3)
        * (s32 - s31)
        / (s22 - s21)
        * (s42 - s41)
...
def DVP_T(
    ...
    if m_nH == 1:
        ...
        Step = np.array(
            [[0, m_nH / 2], [mantle_data[m_source_layer + 1, 5] / 2, 0]],
            dtype=np.complex128,
        )
        matrix_dvp_1, matrix_dvp_2, matrix_dvp_3, ID_SF = DVP_T_cmd2surface(
            mantle_data, depth_s, depth_f, m_source_layer, m_field_layer, m_nH
        )
        if ID_SF == 0:
            matrix_dvp_g = matrix_dvp_1
        ...
        if ID_SF == 0:
            if (depth_f == 0) & (depth_s == 0):
                ...
                Cx1, dx1 = matrix_cd_internal0(
                    matrix_dvp_2,
                    matrix_dvp_3,
                    np.ascontiguousarray(dx0[2, :] + Step[1, :]).reshape(1, -1),
                )
                dx2 = np.linalg.solve(Cx1, dx1)
        else:
            Cx1, dx1 = matrix_cd_internal1(
                matrix_dvp_1,
                matrix_dvp_2,
                tM,
                np.ascontiguousarray(dx0[2, :]).reshape(1, -1),
            )
            dx2 = np.linalg.solve(Cx1, dx1)
        UN = dx2[1, :].real
...
def DVP_S0_tls(
    ...
    matrix_dvp_g = dvp_propagator(
        dvp_propagator(matrix_dvp_1, matrix_dvp_2), matrix_dvp_3
    )
    b0 = inner_core_sol_0(core_data)
    tc = (ppgC @ b0).astype(np.complex128)
    tM = np.zeros((4, 1), dtype=np.complex128)
    ...
    Hs = (
        c * (matrix_dvp_g[0, 1] * tM[1] + matrix_dvp_g[0, 0] * tM[0])
    )  # only loading has conponent. Just one element. no need to do matrix multiplication
    return Hs
...
def DVP_S_tls(
    ...
    if m_nH == 1:
        ...
        Hs[1] = c3[
            0, 0
        ].real  # it seems like c3[:3,0] is real in matlab version. For computation efficiency with numba, I make all complex values
        Ls[1] = c3[1, 0].real
        Ks[1] = -c3[2, 0].real - 1
        Hs[2] = c3[0, 1].real
        Ls[2] = c3[1, 1].real
        ...
        if m_nH <= 100:
            ...
            t1 = (
                np.linalg.solve(
                    (t0 @ matrix_dvp_g[3:, 3:]).T, (matrix_dvp_g[:3, :3] @ tM[:3:]).T
                ).T
                + matrix_dvp_g[:3, 3:]
            )
            u = t1 @ Ta
...
def cal_love_tls(
    ...
    if degree_i == 1:
        degree_list = np.arange(
            iter_start, degree_e + degree_i, degree_i, dtype=np.int64
        )
    else:
...
class Read_models:
    ...
    def cal_gravity(self):
        ...
        v0 = 0
        mass = 0
        gravity_core = np.zeros(
            self.core_data.shape[0],
        )
        gravity_mantle = np.zeros(
            self.mantle_data.shape[0],
        )
...
class DLN:
    ...
    def loves_tls(self, depth_s, depth_f, degree_b=0, degree_e=1000, degree_i=1):
        ...
        loves["Hs"], loves["Ls"], loves["Ks"], loves["H22"], loves["degree_list"] = (
            cal_love_tls(
                self.core_data,
                self.mantle_data,
                depth_s * 1000,
    ...
    def loves_dislocation(
        self, depth_s, depth_f, degree_b=0, degree_e=1000, degree_i=1
    ):
        """
