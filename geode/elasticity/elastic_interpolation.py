...
def gpsgridder(
    ...
    q, p, w = get_qpw(np.column_stack([xi, yi]), np.column_stack([xi, yi]), dr, nu)
    ...
    for row in range(nrows):
        for col in range(ncols):
            ...
            q_eval, p_eval, w_eval = get_qpw(
                xi_yi, np.array([[X[row, col], Y[row, col]]]), dr, nu
            )
            # Sum weighted contributions from all data points
            U[row, col] = (
                u0
                + np.dot(q_eval.flatten(), wt[:n])
                + np.dot(w_eval.flatten(), wt[n:n2])
            )
            V[row, col] = (
                v0
                + np.dot(w_eval.flatten(), wt[:n])
                + np.dot(p_eval.flatten(), wt[n:n2])
            )
...
def get_qpw(
    X: np.ndarray, P: np.ndarray, dr: float, nu: float
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
            ...
            G[id1] = -np.log(0.5 * cx) * (
                ...
                * (
                    ...
                    * (
                        ...
                        * (
                            1.2067492
                            + z * (0.2659732 + z * (0.360768e-1 + z * 0.45813e-2))
                        )
                    )
                )
            ) + (
...
if __name__ == "__main__":
    ...
    print(
        f"Input velocity range: u=[{ui.min():.2f}, {ui.max():.2f}], v=[{vi.min():.2f}, {vi.max():.2f}]"
    )
    print(
        f"Output velocity range: U=[{U.min():.2f}, {U.max():.2f}], V=[{V.min():.2f}, {V.max():.2f}]"
    )
