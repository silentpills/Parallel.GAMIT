...
def build_design_matrix(
    source_data, field_data, fault_para, core_file=None, mantle_file=None, ISO=1
):
    ...
    if not mantle_file:
        ...
        data_path = files("geode.elasticity.data").joinpath("EarthMantle56.txt")
        mantle_file = str(data_path)

    if not core_file:
        from importlib.resources import files

        data_path = files("geode.elasticity.data").joinpath("EarthCore.txt")
    ...
    for i in range(source_data.shape[0]):
        ...
        cos_psi = np.cos(lat_s) * np.cos(field_data[:, 0]) + np.sin(lat_s) * np.sin(
            field_data[:, 0]
        ) * np.cos(lon_s - field_data[:, 1])
        sin_psi = np.sqrt(1 - cos_psi**2)
        psi = np.arctan2(sin_psi, cos_psi)
        sin_A = np.sin(field_data[:, 0]) * np.sin(field_data[:, 1] - lon_s) / sin_psi
        cos_A = (
            (np.cos(field_data[:, 0]) - np.cos(lat_s) * cos_psi)
            / np.sin(lat_s)
            / sin_psi
        )
        ...
        cos_alpha = (
            -(np.cos(lat_s) - np.cos(field_data[:, 0]) * cos_psi)
            / np.sin(field_data[:, 0])
            / sin_psi
        )
        ...
        # Vt_field = [Vt12, Vt32]
        # D11
        design_matrix[::3, 2 * i] = area * (
            -Ur_field[:, 0] * np.sin(2 * angle_B) * np.sin(dip)
            + Ur_field[:, 1] * np.sin(angle_B + half_pi) * np.cos(dip)
        )
        # D12
        design_matrix[::3, 2 * i + 1] = area * (
            0.5
            * (
                Ur_field[:, 3]
                - Ur_field[:, 2]
                - Ur_field[:, 0] * np.sin(2 * angle_B - half_pi)
            )
            * np.sin(2 * dip)
            + Ur_field[:, 1] * np.sin(angle_B) * np.cos(2 * dip)
        )
        # D21
        design_matrix[1::3, 2 * i] = area * (
            cos_alpha
            * (
            ...
            + sin_alpha
            * (
                -Vt_field[:, 0] * np.cos(2 * angle_B) * np.sin(dip)
        ...
        design_matrix[1::3, 2 * i + 1] = area * (
            ...
            * (
                ...
                * (
                    Vs_field[:, 3]
                    - Vs_field[:, 2]
                    - Vs_field[:, 0] * np.sin(2 * angle_B - half_pi)
                )
                * np.sin(2 * dip)
                + Vs_field[:, 1] * np.sin(angle_B) * np.cos(2 * dip)
                - 0.5
                * tVs_field[:, 0]
                * np.sin(2 * angle_B - half_pi)
                * np.sin(2 * dip)
...
def get_green_func(DLN_obj, depth_s, nodal_point, ISO=1):
    ...
    dir_name = os.path.join(
        os.getcwd(),
        f"GF_{os.path.splitext(DLN_obj.core_file)[0]}_{os.path.splitext(DLN_obj.mantle_file)[0]}",
    )
    ...
    if not os.path.isfile(save_name):
        if ISO == 1:
            ...
            if abs(depth_s) > 1:
                ...
                degree_e = 2000000
                degree_i = 100000
            love = DLN_obj.loves_dislocation(
                depth_s, 0, 0, np.round(CONST_R_km * 10 / depth_s), 1
            )
...
def fitDLN(rx, love):
    ...
    love["Hs"][:, ind_normalized] = (
        love["Hs"][:, ind_normalized] * love["degree_list"][:, None]
    )
    love["Ls"][:, ind_normalized] = (
        love["Ls"][:, ind_normalized] * love["degree_list"][:, None]
    )
    love["Ks"][:, ind_normalized] = (
        love["Ks"][:, ind_normalized] * love["degree_list"][:, None]
    )
    love["Lt"][:, 0] = love["Lt"][:, 0] * love["degree_list"][:] ** 2
    love["Lt"][:, 1] = love["Lt"][:, 1] * love["degree_list"][:]
    love_para["Hs"] = np.polyfit(
        x=love["degree_list"][:], y=love["Hs"][:] / scaling[:, None], deg=2
    ).T  # matlab polyfit is slightly different from numpy. so the results are slightly different
    love_para["Ls"] = np.polyfit(
        x=love["degree_list"][:], y=love["Ls"][:] / scaling[:, None], deg=2
    ).T
    love_para["Ks"] = np.polyfit(
        x=love["degree_list"][:], y=love["Ks"][:] / scaling[:, None], deg=2
    ).T
    love_para["Lt"] = np.polyfit(
        x=love["degree_list"][:], y=love["Lt"][:] / scaling[:, None], deg=2
    ).T
    return love_para
...
def cal_green_func(depth_s, love, love_para, nodal_point):
    ...
    rR = (
        rs / CONST_R
    )  # in matlab rR = rx = rs/rf = rf/CONST_R. I just remove unnecessary variables.
    ...
    Ur3_0 = (
        np.sqrt(1 / 4 / np.pi) / rs**2 * love["H33"] * np.sqrt(1 / 4 / np.pi)
    )  # c33 == c22
    ...
    if love_para is not None:
        for i in range(nodal_point.shape[0]):
            ...
            c22 = (
                np.sqrt((2 * love["degree_list"][:] + 1) / np.pi / 4)
                / rs**2
                * pnm[:, 6]
            )
            c32 = c22 / np.sqrt((love["degree_list"][:] + 1) * (love["degree_list"][:]))
            c12 = c32 * np.sqrt(
                (love["degree_list"][:] + 2) * (love["degree_list"][:] - 1)
            )
            # degree == 1~n_max
            tmp = (
                love["Hs"][:, 0]
                - (
                    love["degree_list"][:] * love_para["Hs"][0, 0]
                    + love_para["Hs"][0, 1]
                )
                * rRn
            )
            ...
            tmp = (
                love["Hs"][:, 1]
                - (
                    love["degree_list"][:] ** 2 * love_para["Hs"][1, 0]
                    + love["degree_list"][:] * love_para["Hs"][1, 1]
                    + love_para["Hs"][1, 2]
                )
                * rRn
            )
            ...
            tmp = (
                love["Hs"][:, 2]
                - (
                    love["degree_list"][:] * love_para["Hs"][2, 0]
                    + love_para["Hs"][2, 1]
                )
                * rRn
            )
            ...
            tmp = (
                love["Hs"][:, 3]
                - (
                    love["degree_list"][:] * love_para["Hs"][3, 0]
                    + love_para["Hs"][3, 1]
                )
                * rRn
            )
            ...
            tmp = (
                ...
                - (
                    love_para["Ls"][0, 0]
                    + love_para["Ls"][0, 1] / love["degree_list"][:]
                )
                * rRn
            )
            ...
            tmp = (
                love["Ls"][:, 1]
                - (
                    love["degree_list"][:] * love_para["Ls"][1, 0]
                    + love_para["Ls"][1, 1]
                )
                * rRn
            )
            ...
            tmp = (
                ...
                - (
                    love_para["Ls"][2, 0]
                    + love_para["Ls"][2, 1] / love["degree_list"][:]
                )
                * rRn
            )
            ...
            tmp = (
                ...
                - (
                    love_para["Ls"][3, 0]
                    + love_para["Ls"][3, 1] / love["degree_list"][:]
                )
                * rRn
            )
            ...
            tmp = (
                love["Lt"][:, 0]
                - (love_para["Lt"][0, 0] / love["degree_list"][:]) * rRn
            )
            ...
            Ur[i, 0] = -2 * (
                ...
                + cc
                * (
                    love_para["Hs"][0, 0] * (2 * sums[8] - sums[10])
                    + love_para["Hs"][0, 1] * (sums[9] + sums[10])
                )
            )
            Ur[i, 1] = -2 * (
                Ur[i, 1]
                + cc
                * (
                    love_para["Hs"][1, 0] * (2 * sums[4] - sums[5] + sums[7])
                    + love_para["Hs"][1, 1] * (2 * sums[5] - sums[7])
                    + love_para["Hs"][1, 2] * (sums[6] + sums[7])
                )
            )
            Ur[i, 2] = Ur[i, 2] + cc * (
                love_para["Hs"][2, 0] * (2 * sums[0] + sums[1])
                + love_para["Hs"][2, 1] * (2 * sums[1] + sums[2])
            )
            Ur[i, 3] = Ur[i, 3] + cc * (
                love_para["Hs"][3, 0] * (2 * sums[0] + sums[1])
                + love_para["Hs"][3, 1] * (2 * sums[1] + sums[2])
            )
            ...
            Vs[i, 0] = -2 * (
                ...
                + cc
                * (
                    love_para["Ls"][0, 0] * (sums[19] + sums[20])
                    + love_para["Ls"][0, 1] * (sums[21] + sums[22])
                )
            )
            Vs[i, 1] = -2 * (
                Vs[i, 1]
                + cc
                * (
                    love_para["Ls"][1, 0] * (2 * sums[16] - sums[18])
                    + love_para["Ls"][1, 1] * (sums[17] + sums[18])
                )
            )
            Vs[i, 2] = Vs[i, 2] + cc * (
                love_para["Ls"][2, 0] * (2 * sums[13] + sums[14])
                + love_para["Ls"][2, 1] * (2 * sums[14] + sums[15])
            )
            Vs[i, 3] = Vs[i, 3] + cc * (
                love_para["Ls"][3, 0] * (2 * sums[13] + sums[14])
                + love_para["Ls"][3, 1] * (2 * sums[14] + sums[15])
            )
            ...
            Vt[i, 0] = -4 * (
                ...
                + cc
                * (
                    love_para["Ls"][0, 0] * (sums[9] + sums[10])
                    + love_para["Ls"][0, 1] * (sums[11] + sums[12])
                )
                / np.sin(nodal_point[i])
            )
            Vt[i, 1] = -2 * (
                Vt[i, 1]
                + cc
                * (
                    love_para["Ls"][1, 0] * (2 * sums[5] - sums[7])
                    + love_para["Ls"][1, 1] * (sums[6] + sums[7])
                )
                / np.sin(nodal_point[i])
            )
            ...
            tVs[i, 0] = -4 * (
                tVs[i, 0]
                + cc
                * (love_para["Lt"][0, 0] * (sums[11] + sums[12]))
                / np.sin(nodal_point[i])
            )
            tVs[i, 1] = 2 * (
                tVs[i, 1]
                + cc
                * (love_para["Lt"][1, 0] * (sums[6] + sums[7]))
                / np.sin(nodal_point[i])
            )
            tVt[i, 0] = -2 * (
                tVt[i, 0] + cc * (love_para["Lt"][0, 0] * (sums[21] + sums[22]))
            )
            tVt[i, 1] = 2 * (
                tVt[i, 1] + cc * (love_para["Lt"][1, 0] * (sums[17] + sums[18]))
            )
        return Ur, Vs, Vt, tVs, tVt
    else:
        for i in range(nodal_point.shape[0]):
            ...
            Vt[i, 0] = (c12 * love["Ls"][:, 0] * pnm[:, 2]).sum() / np.sin(
                nodal_point[i]
            )

            Vs[i, 1] = (c32 * love["Ls"][:, 1] * pnm[:, 4]).sum()  # c33 == c22
            Vt[i, 1] = (c32 * love["Ls"][:, 1] * pnm[:, 1]).sum() / np.sin(
                nodal_point[i]
            )
            ...
            # toroidal
            tVs[i, 0] = (c12 * love["Lt"][:, 0] * pnm[:, 2]).sum() / np.sin(
                nodal_point[i]
            )
            tVt[i, 0] = (c12 * love["Lt"][:, 0] * pnm[:, 5]).sum()

            tVs[i, 1] = (c32 * love["Lt"][:, 1] * pnm[:, 1]).sum() / np.sin(
                nodal_point[i]
            )
...
def asso_legendre(theta, n_max):
    ...
    if theta == 0:
        ...
        p[1:, 3:-1] = (
            -p[:-1, :3] * (degree_list[:, np.newaxis] + order_list)
            + p[1:, :3] * x * degree_list[:, np.newaxis]
        ) / y
...
def infinite_sum(theta, t):
    ...
    if theta > 0.002:
        sums[11] = (
            1
            - 1 / sq1
            + ss2**2 / cc2**2 * np.log(tQ / 2)
            + 2
            * x
            * (np.log(1 + 2 * (1 + t) / (x - t + sq1) * ss2**2) + np.log(cc2**2))
            / y**2
        )

        sums[21] = (
            2 * t / y / sq1
            + ss2 / cc2**3 * np.log((x - t + sq1) / 2 / cc2**2)
            - t * y * (1 + 1 / sq1) / tQ
            + 4 * x * (np.log(1 - 2 * (1 + t) / tQ * ss2**2) - np.log(cc2**2)) / y**3
            + t * y / sq3
        )
    else:
        sums[11] = (
    ...
    sums[20] = (
        ...
        * y
        / tQ
        / sq3
        * (t**2 / tQ - sq1 + t / (t - x - sq1) - t * x + t * sq1 / 2 / cc2**2)
        + t * y / sq3
        - 3 * t**2 * y * (t - x) / sq5
    )
