...
def mrectloadhs_dif(spos, xr, yr, lambda_param, mu, return_combined=False):
    ...
    if np.any(spos[2, :] < 0):
        raise ValueError(
            "spos contains stations located above the half space (with z<0)"
        )

    if xr.ndim != 2 or xr.shape[1] != 2:
        raise ValueError(
            "xr must be a matrix with two columns "
            "(and row dimension = number of rectangles)"
        )

    if yr.ndim != 2 or yr.shape[1] != 2:
        raise ValueError(
            "yr must be a matrix with two columns "
            "(and row dimension = number of rectangles)"
        )
    ...
    if yr.shape[0] != m:
        raise ValueError(
            "xr and yr must have same number of rows (one for each rectangle)"
        )

    for j in range(m):
        if xr[j, 0] >= xr[j, 1] or yr[j, 0] >= yr[j, 1]:
            raise ValueError(
                "the first element of each row of xr (or yr) must be "
...
def rectloadhs(spos, xr, yr, P, lambda_param, mu):
    ...
    with warnings.catch_warnings():
        ...
        if xr[0] >= xr[1] or yr[0] >= yr[1]:
            raise ValueError(
                "the first element of xr (or yr) must be smaller than its second element"
            )
        ...
        r10_pb, b10_pb, p10_pb, r20_pb, b20_pb, p20_pb = _love_10_20(
            x, y, z, a, b, +b
        )  # y' = +b (& x'=a)
        r10_mb, b10_mb, p10_mb, r20_mb, b20_mb, p20_mb = _love_10_20(
            x, y, z, a, b, -b
        )  # y' = -b (& x'=a)
        r01_pa, b01_pa, p01_pa, r02_pa, b02_pa, p02_pa = _love_01_02(
            x, y, z, a, +a, b
        )  # x' = +a (& y'=b)
        r01_ma, b01_ma, p01_ma, r02_ma, b02_ma, p02_ma = _love_01_02(
            x, y, z, a, -a, b
        )  # x' = -a (& y'=b)

        # Compute functions J1 and J2 for cases y' = +b and -b
        # (where _pb indicates +b and _mb indicates -b)
        J1_pb, J2_pb = _love_J1_J2(
            r10_pb, p10_pb, b10_pb, r20_pb, p20_pb, b20_pb, a, x, y, z, +b
        )
        J1_mb, J2_mb = _love_J1_J2(
            r10_mb, p10_mb, b10_mb, r20_mb, p20_mb, b20_mb, a, x, y, z, -b
        )

        # Compute functions K1 and K2 for cases x' = +a and -a
        # (where _pa indicates +a and _ma indicates -a)
        K1_pa, K2_pa = _love_K1_K2(
            r01_pa, p01_pa, b01_pa, r02_pa, p02_pa, b02_pa, b, x, y, z, +a
        )
        K1_ma, K2_ma = _love_K1_K2(
            r01_ma, p01_ma, b01_ma, r02_ma, p02_ma, b02_ma, b, x, y, z, -a
        )

        # Compute functions L1 and L2 for cases y' = +b and -b
        # (where _pb indicates +b and _mb indicates -b)
        L1_pb, L2_pb = _love_L1_L2(
            r10_pb, p10_pb, b10_pb, r20_pb, p20_pb, b20_pb, a, x, y, z, +b
        )
        L1_mb, L2_mb = _love_L1_L2(
            r10_mb, p10_mb, b10_mb, r20_mb, p20_mb, b20_mb, a, x, y, z, -b
        )
