...
try:
    ...
    logger.warning(
        "netCDF4 not available - GGXF format support disabled. "
...
def read_ggxf_grid(
    ...
    if not GGXF_AVAILABLE:
        raise ImportError(
            "netCDF4 library required for GGXF support. Install with: pip install netCDF4"
        )
    ...
    with Dataset(filename, "r") as nc:
        ...
        if "latitude" in nc.dimensions and "longitude" in nc.dimensions:
            ...
            raise ValueError(
                "GGXF file must have 'latitude'/'longitude' or 'lat'/'lon' dimensions"
            )
        ...
        if east is None or north is None:
            ...
            raise ValueError(
                f"Could not find east/north components. Available variables: {available}"
            )
...
def write_ggxf_grid(
    ...
    with Dataset(filename, "w", format="NETCDF4") as nc:
        ...
        nc.source = metadata.get(
            "source", "Elastic interpolation (Sandwell & Wessel 2016)"
        )
        nc.history = f"{datetime.now().isoformat()}: Created by ElasticGridder"
        nc.references = metadata.get(
            "references", "Sandwell & Wessel (2016) doi:10.1002/2016GL070340"
        )
        nc.comment = metadata.get(
            "description", "Gridded geodetic data from elastic interpolation"
        )
...
class ElasticGridder:
    ...
    def create_velocity_constraint(
        self, grid_file: str, model_type: str = "poly", sigma: float = 0.001
    ) -> "EtmFunction":
        """
        ...
        logger.info(
            f"Grid velocity range: ve=[{ve_flat.min():.2f}, {ve_flat.max():.2f}], "
        ...
        logger.info(
            f"Interpolated velocity at station: ve={ve_interp[0]:.2f}, vn={vn_interp[0]:.2f} mm/yr"
        )
        ...
        if model_type == "poly":
            ...
            funct.p.params[0] = np.array(
                [np.nan, vn_interp[0] / 1000]
            )  # north, mm/yr -> m/yr
            funct.p.params[1] = np.array(
                [np.nan, ve_interp[0] / 1000]
            )  # east, mm/yr -> m/yr
            ...
            logger.info(
                "Created PolynomialFunction constraint with velocity constraint"
            )
        else:
    ...
    def create_jump_constraint(
        ...
        logger.info(
            f"Grid displacement range: de=[{de_grid.min():.2f}, {de_grid.max():.2f}], "
        ...
        logger.info(
            f"Interpolated displacement: de={de_interp[0]:.2f}, dn={dn_interp[0]:.2f} mm"
        )
        ...
        if relaxation is not None:
            ...
            logger.info("Created JumpFunction (coseismic only)")

        return funct

    def sample_grid_at_locations(
        self, grid_file: str, lons: np.ndarray, lats: np.ndarray
        ...
        x_grid, y_grid = azimuthal_equidistant(
            lon_center, lat_center, model_grid[:, 0], model_grid[:, 1]
        )

        # Project sample locations
        x_sample, y_sample = azimuthal_equidistant(lon_center, lat_center, lons, lats)
    ...
    def create_ggxf_from_samples(
        ...
        if not GGXF_AVAILABLE:
            raise ImportError(
                "netCDF4 library required for GGXF export. "
                "Install with: pip install netCDF4"
            )
        ...
        logger.info(
            f"Output velocity range: ve=[{ve_grid.min():.2f}, {ve_grid.max():.2f}], "
        ...
        metadata["institution"] = metadata.get(
            "institution", "Generated by ElasticGridder"
        )
...
def create_elastic_constraint_from_grid(
    config: "EtmConfig",
    grid_file: str,
    model_type: str = "poly",
    dr: float = 8.0,
