...
# app
from geode.pyETM import Periodic, Polynomial
...
class Ztd(object):
    def __init__(self, cnn, NetworkCode, StationCode, project, plotit=False):
        ...
        self.polynomial = Polynomial(
            cnn, NetworkCode, StationCode, self.soln, self.soln.t
        )
        self.periodic = Periodic(cnn, NetworkCode, StationCode, self.soln, self.soln.t)

        shape = (
            self.polynomial.design.shape[0],
            self.polynomial.param_count + self.periodic.param_count,
        )
        ...
        self.periodic.column_index = np.arange(
            col_index, col_index + self.periodic.param_count
        )
        ...
        # continuous solution
        shape = (
            self.soln.ts.shape[0],
            self.polynomial.param_count + self.periodic.param_count,
        )
        self.As = np.ndarray(shape)
        self.As[:, self.polynomial.column_index] = self.polynomial.get_design_ts(
            self.soln.ts
        )
        self.As[:, self.periodic.column_index] = self.periodic.get_design_ts(
            self.soln.ts
        )
    ...
    def plot(
        ...
        if residuals:
            ax.plot(
                self.soln.t, self.soln.ztd - np.dot(self.A, self.C), "ob", markersize=2
            )
        else:
