...
from geode import pyBunch, pyDate
...
from geode.etm.core.type_declarations import EtmSolutionType, SolutionType

# app
from geode.metadata.station_info import (
    StationInfo,
    StationInfoHeightCodeNotFound,
    StationInfoNoRecordFound,
)
...
class Station(object):
    def __init__(self, cnn, NetworkCode, StationCode, dates, StationAlias=None):
        ...
        if len(rs) != 0:
            ...
            self.good_rinex = [
                pyDate.Date(year=r["y"], doy=r["d"]) for r in rs if r["c"] >= COMPLETION
            ]
            ...
            for date in self.good_rinex:
                try:
                    ...
                    tqdm.write(
                        f"    WARNING: Observations for day {date.yyyymmdd()} ({date.yyyyddd()}) "
            ...
            raise ValueError(
                "Specified station %s.%s could not be found"
                % (NetworkCode, StationCode)
            )
    ...
    def check_gamit_soln(self, cnn, project, date):
        ...
        """
        soln = cnn.query_float(
            'SELECT * FROM gamit_soln WHERE "Project" = \'%s\' AND "Year" = %i AND '
    ...
    def __eq__(self, station):
        if not isinstance(station, Station):
            raise StationException(
                "type: "
                + str(type(station))
                + " invalid. Can only compare pyStation.Station"
            )

        return (
            self.NetworkCode == station.NetworkCode
            and self.StationCode == station.StationCode
        )
...
class StationInstance(object):
    def __init__(self, cnn, archive, station: Station, date, GamitConfig, is_tie=False):
        ...
        try:
            self.StationInfo = StationInfo(
                cnn, station.NetworkCode, station.StationCode, date
            ).return_stninfo()
            ...
            self.StationInfo = StationInfo(
                cnn, station.NetworkCode, station.StationCode, date
            ).return_stninfo(no_dharp_translate=True)

        self.date = date  # type: pyDate.Date
        self.Archive_path = GamitConfig.archive_path

        # get the APR and sigmas for this date
        station.etm.config.modeling.sigma_floor_h = float(
            GamitConfig.gamitopt["sigma_floor_h"]
        )
        station.etm.config.modeling.sigma_floor_v = float(
            GamitConfig.gamitopt["sigma_floor_v"]
        )
        ...
        # rinex file
        self.archive_file = archive.build_rinex_path(
            self.NetworkCode, self.StationCode, self.date.year, self.date.doy
        )

        # DDG: force RINEX 2 filenames even with RINEX 3 data
        self.filename = (
            self.StationAlias + self.date.ddd() + "0." + self.date.yyyy()[2:4] + "d.Z"
        )
        ...
        rs = cnn.query_float(
            ...
            '"Year" = %s AND "DOY" = %s'
            % (self.NetworkCode, self.StationCode, self.date.yyyy(), self.date.ddd()),
            as_dict=True,
        )
    ...
    def DebugCoord(self):
        ...
        if self.ppp is not None:
            return (
                "%s %s_GPS %8.3f %8.3f %8.3f %14.3f %14.3f %14.3f %8.3f %8.3f %8.3f %8.4f %s"
                % (
                    self.StationAlias.upper(),
                    self.StationAlias.upper(),
                    self.ppp["X"] - self.X,
    ...
    def GetStationInformation(self):
        return self.StationInfo.replace(
            self.StationCode.upper(), self.StationAlias.upper()
        )
...
class StationCollection(list):
    ...
    def compare_aliases(self, station):
        ...
        for stn in self:
            if stn != station and station.StationAlias in (
                stn.StationAlias,
                stn.StationCode,
            ):
                # not unique!
    ...
    def get_active_coordinates(self, date):
        ...
        """
        return np.array(
            [[stn.X, stn.Y, stn.Z] for stn in self if date in stn.good_rinex]
        )
        ...
        if isinstance(item, str):
            for stn in self:
                if stn.netstn == item.lower():
                    return stn
            raise StationCollectionException(
                "Requested station code " + item.lower() + " could not be found"
            )
            ...
            for stn in self:
                if stn.netstn == item.netstn:
                    return stn
            raise StationCollectionException(
                "Requested station code " + item.netstn + " could not be found"
            )
