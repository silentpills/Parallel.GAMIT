...
class PolynomialFunction(EtmFunction):
    ...
    def initialize(
        self, models: tuple = (), time_vector: np.ndarray = np.array([]), **kwargs
    ) -> None:
        ...
        logger.info(
            f"Polynomial -> Fitting {self.config.modeling.poly_terms} term(s), conventional "
        ...
        self.p.param_metadata = (
            f"polynomial:[n:[{params}]],[e:[{params}]],[u:[{params}]]]"
        )
    ...
    def print_parameters(
        self, ce_position: List[np.ndarray] = None
    ) -> Tuple[list, list, list]:
        self.format_str = []

        params = np.array(self.p.params) * 1000.0
        sigmas = np.array(self.p.sigmas) * 1000.0

        self.format_str = (
            self.config.get_label("position") + f" ({self.p.t_ref:.3f}) "
        ...
        if self.param_count > 1:
            self.format_str += (
                ...
                rf"N: {params[0, 1]:.2f} $\pm$ {sigmas[0, 1]:.2f} "
                rf"E: {params[1, 1]:.2f} $\pm$ {sigmas[1, 1]:.2f} "
                rf"U: {params[2, 1]:.2f} $\pm$ {sigmas[2, 1]:.2f} [mm/yr]"
            )

        if self.param_count > 2:
            self.format_str += (
                "\n" + self.config.get_label("acceleration") + " "
                rf"N: {params[0, 2]:.2f} $\pm$ {sigmas[0, 2]:.2f} "
                rf"E: {params[1, 2]:.2f} $\pm$ {sigmas[1, 2]:.2f} "
                rf"U: {params[2, 2]:.2f} $\pm$ {sigmas[2, 2]:.2f} [mm/yr**2]"
            )

        if self.param_count > 3:
            self.format_str += f" + {self.param_count - 3} " + self.config.get_label(
                "other"
            )
    ...
    def rehash(self) -> None:
        ...
        hash_input = (
            f"{self.config.modeling.poly_terms}_{self.config.modeling.reference_epoch}"
        )
