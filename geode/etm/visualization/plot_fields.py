...
def mask_ocean_points(lon, lat, buffer_distance=0.0):
    ...
    if buffer_distance > 0:
        ...
        world_proj["geometry"] = world_proj.geometry.buffer(buffer_distance * 1000.0)
...
def plot_velocity_field(
    ...
    def process_data(index):
        ...
        ve = data[index][0, :] * 1000.0  # East
        vn = data[index][1, :] * 1000.0  # North
        vu = data[index][2, :] * 1000.0  # Up

        ve_stn = stations_data[index][0, :] * 1000.0  # East
        vn_stn = stations_data[index][1, :] * 1000.0  # North
    ...
    def update_scale(val):
        ...
        for i in range(2):
            ...
            if plot_state["plot_sigmas"]:
                ...
                q = add_quiver_key(
                    ax, q_grid, plot_state["reference_vector"], "mm[/yr]"
                )
...
def plot_axes(
    ...
    cs_east = ax.contourf(
        xi_grid, yi_grid, field, levels=levels, cmap=cmap, extend=colorbar_extend
    )
    ...
    if index == 0:
        add_interactive_labels(
            ax, x_stn, y_stn, stations_names, on_station_click, connect
        )
...
def add_interactive_labels(
    ax, x, y, station_names, on_click_callback=None, connect=True
):
    """
    ...
    scatter = ax.scatter(
        x, y, s=20, c="red", marker="^", alpha=0.6, zorder=100, picker=True
    )
