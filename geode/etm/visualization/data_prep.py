...
from ..core.type_declarations import FitStatus, JumpType
...
class PlotDataPreparer:
    ...
    def prepare_time_series_data(
        self, solution_data: SolutionData, etm_fit: EtmFit = None
    ) -> TimeSeriesPlotData:
        ...
        n_data, e_data, u_data = self._prepare_component_data(
        ...
        if etm_fit:
            ...
            plot_data.wrms_values = (
                etm_fit.results[0].wrms * 1000.0,
                etm_fit.results[1].wrms * 1000.0,
                etm_fit.results[2].wrms * 1000.0,
            )
    ...
    def _prepare_component_data(
        ...
        for i in range(3):
            ...
            if etm_results.config.modeling.status == FitStatus.POSTFIT:
                model_values = (
                    etm_results.design_matrix.alternate_time_vector(
                        solution_data.time_vector_cont
                    )
                    @ etm_results.results[i].parameters
                ) + etm_results.results[i].stochastic_signal
                data[i].model_time_vector = solution_data.time_vector_cont
                data[i].model_values = model_values * 1000  # Convert to mm

                # Add residuals
                data[i].residuals = etm_results.results[i].residuals * 1000.0

                for funct in etm_results.design_matrix.functions:
                    if (
                        (
                            funct.p.object == "periodic"
                            and self.output_config.plot_remove_periodic
                        )
                        or (
                            funct.p.object == "polynomial"
                            and self.output_config.plot_remove_polynomial
                        )
                        or (
                            funct.p.object == "jump"
                            and self.output_config.plot_remove_jumps
                        )
                    ) and funct.fit:
                        data[i].observations -= funct.eval(i) * 1000.0
                        data[i].observations_fit -= (
                            funct.eval(i, data[i].time_vector_fit) * 1000.0
                        )
                        if np.any(~mask):
                            data[i].observations_not_fit -= (
                                funct.eval(i, data[i].time_vector_not_fit) * 1000.0
                            )
                        data[i].model_values -= (
                            funct.eval(i, solution_data.time_vector_cont) * 1000.0
                        )
                    ...
                    if self.output_config.plot_remove_stochastic:
                        if funct.p.object == "stochastic":
                            ...
                            data[i].observations -= (
                                funct.eval(i, solution_data.time_vector_mjd) * 1000.0
                            )
                            data[i].observations_fit -= (
                                funct.eval(i, solution_data.time_vector_mjd[mask])
                                * 1000.0
                            )
                            if np.any(~mask):
                                data[i].observations_not_fit -= (
                            ...
                            data[i].model_values -= (
                                etm_results.results[i].stochastic_signal * 1000.0
                            )
