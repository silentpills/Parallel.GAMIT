...
class EtmStackerVCE:
    ...
    def run_vce(
        ...
        for iteration in range(self.max_iterations):
            ...
            for group_name, residual_info in residual_groups.items():
                ...
                if redundancy > 0:
                    ...
                    sigma2_new = 1.0
                    tqdm.write(
                        f"  WARNING: {group_name} has zero redundancy, setting σ²=1"
                    )
                ...
                tqdm.write(
                    f"  {group_name:40s}: σ²={sigma2_new:8.4f}  "
        ...
        if not converged:
            ...
            tqdm.write("    Consider increasing max_iterations or relaxing tolerance\n")

        return self.history[-1]

    @staticmethod
    def _check_convergence(
        variance_factors_old: Dict[str, float],
    ...
    def _initialize_variance_factors(
        self, initial_weights: Optional[Dict[str, float]]
    ) -> Dict[str, float]:
    ...
    def _apply_variance_factors_as_weights(self, variance_factors: Dict[str, float]):
        ...
        self.stacker.update_weights(
            constraint_type="interseismic", h_sigma=h_sigma, v_sigma=v_sigma
        )
        ...
        for event in self.stacker.earthquakes:
            ...
            for relax in self.stacker.config.relaxation:
                ...
                h_sigma = self.stacker.config.postseismic_h_sigma * np.sqrt(
                    postseis_var
                )
                v_sigma = self.stacker.config.postseismic_v_sigma * np.sqrt(
                    postseis_var
                )
                self.stacker.update_weights(
                    event_id=event.id,
                    constraint_type="postseismic",
                    h_sigma=h_sigma,
                    v_sigma=v_sigma,
                    relax=relax,
                )
    ...
    def get_final_weights(self) -> Dict[str, Dict[str, float]]:
        ...
        weights = {"interseismic": {}, "coseismic": {}, "postseismic": {}}
        ...
        for comp_name, component in final_result.components.items():
            if comp_name == "observations":
                ...
                for const in self.stacker.constraint_registry.constraints[
                    "interseismic"
                ]:
    ...
    def print_summary(self):
        ...
        for comp_name in sorted(final_result.components.keys()):
            ...
            print(
                f"{comp_name:<40s} {comp.sigma2:>10.4f} {comp.chi2:>12.2f} {comp.redundancy:>12d}"
            )
...
class EtmStackerVCEEnhanced(EtmStackerVCE):
    ...
    def run_vce_with_observation_scaling(
        self, initial_obs_variance: float = 1.0, scale_per_station: bool = False
    ) -> VCEResults:
        """
    ...
    def _run_vce_global_obs_variance(self, initial_obs_variance: float) -> VCEResults:
        ...
        for iteration in range(self.max_iterations):
            ...
            tqdm.write(
                f"  {'observations':45s}: σ²={obs_variance_new:8.4f}  "
            ...
            for group_name, residual_info in residual_groups.items():
                ...
                tqdm.write(
                    f"  {group_name:45s}: σ²={sigma2_new:8.4f}  "
                    f"(Δ={change:8.4f})  χ²={chi2:10.2f}  r={redundancy:6d}"
                )
            ...
            max_change = self._compute_max_change(
                all_old, all_new, fix_observations=False
            )
        ...
        tqdm.write(f"\n{'=' * 70}")
        tqdm.write("RECOMMENDATION:")
        tqdm.write(f"{'=' * 70}")
        tqdm.write(
            f"Set observation_variance_factor = {final_obs_variance:.4f} in your config"
        )
        tqdm.write(
            f"This will scale all observation weights by 1/{final_obs_variance:.4f}"
        )
        tqdm.write("Then re-run VCE with fix_observations=True for constraint weights")
        tqdm.write(f"{'=' * 70}\n")
    ...
    def _apply_observation_variance(self, obs_variance: float):
        ...
        for stn in self.stacker.stations:
            ...
            self.stacker.change_station_weight(
                stationID(stn), scale_factor, silent=True
            )
    ...
    def _run_vce_per_station_variance(self, initial_obs_variance: float) -> VCEResults:
        ...
        station_variances = {
            neq.station: initial_obs_variance for neq in self.stacker.normal_equations
        }
        ...
        for iteration in range(self.max_iterations):
            ...
            for stn in self.stacker.stations:
                ...
                for i in range(3):
                    neq.observation_weights[i] = (
                        orig["observation_weights"][i] * scale_factor
                    )
                    p_scaled = np.diag(neq.observation_weights[i])
                    neq.neq[i] = neq.design_matrix.T @ p_scaled @ neq.design_matrix
                    neq.ceq[i] = (
                        neq.design_matrix.T @ p_scaled @ neq.observation_vector[i]
                    )
                    neq.weighted_observations[i] = (
                        neq.observation_vector[i].T
                        @ p_scaled
                        @ neq.observation_vector[i]
                    )
            ...
            top_stations = sorted(
                station_variances_new.items(), key=itemgetter(1), reverse=True
            )[:10]
            ...
            max_station_change = max(
                abs(station_variances_new[sid] - station_variances[sid])
                / station_variances[sid]
        ...
        tqdm.write(f"\n{'=' * 70}")
        tqdm.write("FINAL STATION VARIANCE STATISTICS:")
        tqdm.write(f"{'=' * 70}")
...
def quick_observation_variance_check(stacker):
    ...
    for event in stacker.earthquakes:
        stacker.update_weights(
            event_id=event.id, constraint_type="coseismic", h_sigma=100.0, v_sigma=100.0
        )
        stacker.update_weights(
            event_id=event.id,
            constraint_type="postseismic",
    ...
    print("With 100m constraint sigmas:")
    ...
    print("2. This will downweight observations to match network constraints")
    print("3. Then run VCE with fix_observations=True for fine-tuning")
    print("=" * 70 + "\n")
