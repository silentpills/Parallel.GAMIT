            ...
            if record[0] == " " and len(record) >= 77:
                record = dict(
                    zip(
                        ...
                        map(
                            str.strip,
                            struct_unpack(fieldstruct, record.ljust(fieldstruct.size))[
                                1:
                            ],
                        ),
                    )
...
class StationInfo:
    ...
    def __init__(
        ...
        self.header = (
            "*SITE  Station Name      Session Start      Session Stop       Ant Ht   HtCod  Ant N    "
    ...
    def load_stationinfo_records(self):
        ...
        if stninfo.ntuples() == 0:
            if not self.allow_empty:
                ...
                raise pyStationInfoException(
                    "Could not find ANY valid station info entry for " + stationID(self)
                )
            self.record_count = 0
            return False
        else:
            for record in stninfo.dictresult():
                self.records.append(
                    StationInfoRecord(self.NetworkCode, self.StationCode, record)
                )
    ...
    def station_info_gaps(self):
        ...
        rs = self.cnn.query(
            ...
            'AND "Completion" >= 0.5' % (self.NetworkCode, self.StationCode)
        )  # only check RINEX with more than 12 hours of data
                ...
                for row in logfile:
                    stninfo.append(
                        fs.format(
                            ...
                            str(pyDate.Date(datetime=row[3]))
                            if row[3].year < 2100
                            else "9999 999 00 00 00",  # session end
                            float(row[4])
                            if type(row[4]) is float
                            else 0.000,  # antenna height
                            row[5],  # height code
                            float(row[6])
                            if type(row[6]) is float
                            else 0.000,  # antenna north offset
                            float(row[7])
                            if type(row[7]) is float
                            else 0.000,  # antenna east offset
                            row[8],  # receiver type
    ...
    def to_dharp(self, record):
        ...
        if record.HeightCode == "DHARP":
            ...
            htc = self.cnn.query_float(
                "SELECT * FROM gamit_htc WHERE \"AntennaCode\" = '%s' AND \"HeightCode\" = '%s'"
                % (record.AntennaCode, record.HeightCode),
                as_dict=True,
            )

            if len(htc):
                record.AntennaHeight = np.sqrt(
                    np.square(float(record.AntennaHeight))
                    - np.square(float(htc[0]["h_offset"]))
                ) - float(htc[0]["v_offset"])
                ...
                raise pyStationInfoHeightCodeNotFound(
                    "%s: %s -> Could not translate height code %s to DHARP. "
                    "Check the height codes table."
                    % (stationID(self), record.AntennaCode, record.HeightCode)
                )
    ...
    def return_stninfo_short(self, record=None):
        ...
        return "\n".join(
            " %s.%s [...] %s" % (self.NetworkCode.upper(), l[1:110], l[160:])
    ...
    def UpdateStationInfo(self, record, new_record):
        ...
        if self.NetworkCode and self.StationCode:
            ...
            if (
                new_record["DateStart"].datetime() - record["DateStart"].datetime()
            ).seconds != 0:
                self.cnn.query(
            ...
            if rs.ntuples() == 0:
                ...
                if overlaps:
                    ...
                    if (
                        ...
                        and record["DateStart"].datetime()
                        < self.records[0]["DateStart"].datetime()
                    ):
                        if self.records_are_equal(record, self.records[0]):
                            ...
                            # insert event
                            event = pyEvents.Event(
                                Description="The start date of the station information record "
                                + self.records[0]["DateStart"].strftime()
                        ...
                        raise pyStationInfoException(
                            "Record %s -> %s overlaps with existing station.info records: %s"
                            % (
                                str(record["DateStart"]),
                                str(record["DateEnd"]),
                                " ".join(stroverlap),
                ...
                raise pyStationInfoException(
                    "Record %s -> %s already exists in station.info"
    ...
    def rinex_based_stninfo(self, ignore):
        ...
        rs = self.cnn.query(
            'SELECT * FROM rinex WHERE "NetworkCode" = \''
            + self.NetworkCode
            + "' AND \"StationCode\" = '"
            + self.StationCode
        ...
        for i, rnx in enumerate(rnxtbl):
            if (
                RecSerial != rnx["ReceiverSerial"]
                or AntSerial != rnx["AntennaSerial"]
                or AntHeig != rnx["AntennaOffset"]
                or RadCode != rnx["AntennaDome"]
            ):
                # start the counter
                count += 1
                ...
                RecSerial == rnx["ReceiverSerial"]
                and AntSerial == rnx["AntennaSerial"]
                and AntHeig == rnx["AntennaOffset"]
                and RadCode == rnx["AntennaDome"]
                and count > 0
            ):
    ...
    def __eq__(self, stninfo):
        ...
        return (
            self.currentrecord.AntennaCode == stninfo.currentrecord.AntennaCode
            and self.currentrecord.AntennaHeight == stninfo.currentrecord.AntennaHeight
            and self.currentrecord.AntennaNorth == stninfo.currentrecord.AntennaNorth
            and self.currentrecord.AntennaEast == stninfo.currentrecord.AntennaEast
            and self.currentrecord.AntennaSerial == stninfo.currentrecord.AntennaSerial
            and self.currentrecord.ReceiverCode == stninfo.currentrecord.ReceiverCode
            and self.currentrecord.ReceiverSerial
            == stninfo.currentrecord.ReceiverSerial
            and self.currentrecord.RadomeCode == stninfo.currentrecord.RadomeCode
        )
