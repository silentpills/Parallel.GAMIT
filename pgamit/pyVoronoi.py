...
def filter_tetrahedron_to_triangle(current_tetrahedron_coord_array):
    ...
    for row in (
        current_tetrahedron_coord_array
    ):  # ugly to use for loop for this, but ok for now!
...
def test_polygon_for_self_intersection(array_ordered_Voronoi_polygon_vertices_2D):
    ...
    list_vertex_indices_in_edges = [
        [vertex_index, vertex_index + 1] for vertex_index in range(total_vertices)
    ]
    ...
    for list_vertex_indices_in_edge in list_vertex_indices_in_edges:
        ...
        filtered_list_vertex_indices_in_edges.append(
            filtered_list_vertex_indices_in_edge
        )

    for edge_index, list_vertex_indices_in_edge in enumerate(
        filtered_list_vertex_indices_in_edges
    ):
        for edge_index_2, list_vertex_indices_in_edge_2 in enumerate(
            filtered_list_vertex_indices_in_edges
        ):
            if (
                list_vertex_indices_in_edge[0] not in list_vertex_indices_in_edge_2
            ) and (
                list_vertex_indices_in_edge[1] not in list_vertex_indices_in_edge_2
            ):  # non-consecutive edges
                a = array_ordered_Voronoi_polygon_vertices_2D[
                    list_vertex_indices_in_edge[0]
                ]
                b = array_ordered_Voronoi_polygon_vertices_2D[
                    list_vertex_indices_in_edge[1]
                ]
                c = array_ordered_Voronoi_polygon_vertices_2D[
                    list_vertex_indices_in_edge_2[0]
                ]
                d = array_ordered_Voronoi_polygon_vertices_2D[
                    list_vertex_indices_in_edge_2[1]
                ]
...
def calculate_Vincenty_distance_between_spherical_points(
    ...
    radian_angle = math.atan2(
        math.sqrt(
            (math.sin(phi_2) * math.sin(delta_lambda)) ** 2
            + (
                math.sin(phi_1) * math.cos(phi_2)
                - math.cos(phi_1) * math.sin(phi_2) * math.cos(delta_lambda)
            )
            ** 2
        ),
        (
            math.cos(phi_1) * math.cos(phi_2)
            + math.sin(phi_1) * math.sin(phi_2) * math.cos(delta_lambda)
        ),
    )
...
def calculate_haversine_distance_between_spherical_points(
    ...
    spherical_distance = (
        ...
        * math.asin(
            math.sqrt(
                ((1 - math.cos(phi_2 - phi_1)) / 2.0)
                + math.sin(phi_1)
                * math.sin(phi_2)
                * ((1 - math.cos(lambda_2 - lambda_1)) / 2.0)
            )
        )
    )
    return spherical_distance
...
def filter_polygon_vertex_coordinates_for_extreme_proximity(
    ...
    while 1:
        ...
        threshold_assessment_matrix = (
            distance_matrix / maximum_euclidean_distance_between_any_vertices
        )
        row_indices_that_violate_threshold, column_indices_that_violate_threshold = (
            numpy.where(
                (threshold_assessment_matrix < vertex_merge_threshold)
                & (threshold_assessment_matrix > 0)
            )
        )
        if (
            len(row_indices_that_violate_threshold) > 0
            and len(column_indices_that_violate_threshold) > 0
        ):
            for row, column in zip(
                row_indices_that_violate_threshold,
                column_indices_that_violate_threshold,
            ):
                if not row == column:  # ignore diagonal values
                    ...
                    new_vertex_at_midpoint = (
                        array_ordered_Voronoi_polygon_vertices[row]
                        + array_ordered_Voronoi_polygon_vertices[column]
                    ) / 2.0
                    ...
                    spherical_polar_coords_new_vertex[0] = (
                        sphere_radius  # project back to surface of sphere
                    )
                    ...
                    array_ordered_Voronoi_polygon_vertices = numpy.delete(
                        array_ordered_Voronoi_polygon_vertices, column, 0
                    )
                    break
        else:
...
def calculate_surface_area_of_planar_polygon_in_3D_space(
    array_ordered_Voronoi_polygon_vertices,
):
...
def calculate_surface_area_of_a_spherical_Voronoi_polygon(
    ...
    spherical_coordinates = convert_cartesian_array_to_spherical_array(
        array_ordered_Voronoi_polygon_vertices
    )
    spherical_coordinates[..., 0] = 1.0
    array_ordered_Voronoi_polygon_vertices = convert_spherical_array_to_cartesian_array(
    ...
    if scipy.spatial.distance.pdist(array_ordered_Voronoi_polygon_vertices).min() < (
        10**-7
    ):
        return 10**-8
    else:
        ...
        root_b_dist = calculate_haversine_distance_between_spherical_points(
        ...
        for i in 1 + numpy.arange(n - 2):
            ...
            a_b_dist = calculate_haversine_distance_between_spherical_points(
                a_point, b_point, 1.0
            )
            s = (root_a_dist + root_b_dist + a_b_dist) / 2
            totalexcess += 4 * math.atan(
                math.sqrt(
                    math.tan(0.5 * s)
...
def calculate_and_sum_up_inner_sphere_surface_angles_Voronoi_polygon(
    array_ordered_Voronoi_polygon_vertices, sphere_radius
):
    ...
    num_vertices_in_Voronoi_polygon = array_ordered_Voronoi_polygon_vertices.shape[
        0
    ]  # the number of rows == number of vertices in polygon
    ...
    while current_vertex_index < num_vertices_in_Voronoi_polygon:
        current_vertex_coordinate = array_ordered_Voronoi_polygon_vertices[
        ...
        a = calculate_Vincenty_distance_between_spherical_points(
            current_vertex, next_vertex, sphere_radius
        )
        b = calculate_Vincenty_distance_between_spherical_points(
            next_vertex, previous_vertex, sphere_radius
        )
        c = calculate_Vincenty_distance_between_spherical_points(
            previous_vertex, current_vertex, sphere_radius
        )
        # print 'law of haversines a,b,c:', a,b,c
        # print 'Vincenty edge lengths a,b,c:', a,b,c
        pre_acos_term = (math.cos(b) - math.cos(a) * math.cos(c)) / (
            math.sin(a) * math.sin(c)
        )
        if abs(pre_acos_term) > 1.0:
            print(
                "angle calc vertex coords (giving acos violation):",
                [
                    convert_cartesian_array_to_spherical_array(vertex)
                    for vertex in [current_vertex, previous_vertex, next_vertex]
                ],
            )
        ...
        list_Voronoi_poygon_angles_radians.append(
            current_vertex_inner_angle_on_sphere_surface
        )
...
def convert_cartesian_array_to_spherical_array(coord_array, angle_measure="radians"):
    ...
    spherical_coord_array[..., 2] = numpy.arccos(
        coord_array[..., 2] / spherical_coord_array[..., 0]
    )
...
def convert_spherical_array_to_cartesian_array(
    spherical_coord_array, angle_measure="radians"
):
...
def produce_triangle_vertex_coordinate_array_Delaunay_sphere(hull_instance):
    ...
    for simplex in (
        hull_instance.simplices
    ):  # for each simplex (face; presumably a triangle) of the convex hull
        convex_hull_triangular_facet_vertex_coordinates = hull_instance.points[simplex]
        assert convex_hull_triangular_facet_vertex_coordinates.shape == (3, 3), (
            "Triangular facet of convex hull should be a triangle in 3D space specified by coordinates in a shape (3,3) numpy array."
        )
        list_points_vertices_Delaunay_triangulation.append(
            convex_hull_triangular_facet_vertex_coordinates
        )
    array_points_vertices_Delaunay_triangulation = numpy.array(
        list_points_vertices_Delaunay_triangulation
    )
    return array_points_vertices_Delaunay_triangulation


def produce_array_Voronoi_vertices_on_sphere_surface(
    facet_coordinate_array_Delaunay_triangulation, sphere_radius, sphere_centroid
):
    ...
    assert facet_coordinate_array_Delaunay_triangulation.shape[1:] == (3, 3), (
        "facet_coordinate_array_Delaunay_triangulation should have shape (N,3,3)."
    )
    # draft numpy vectorized workflow to avoid Python for loop
    facet_normals_array = numpy.cross(
        facet_coordinate_array_Delaunay_triangulation[..., 1, ...]
        - facet_coordinate_array_Delaunay_triangulation[..., 0, ...],
        facet_coordinate_array_Delaunay_triangulation[..., 2, ...]
        - facet_coordinate_array_Delaunay_triangulation[..., 0, ...],
    )
    ...
    triangle_centroid_array = numpy.average(
        facet_coordinate_array_Delaunay_triangulation, axis=1
    )
    # normalize the triangle_centroid to unit sphere distance for the purposes of the following directionality check
    array_triangle_centroid_spherical_coords = (
        convert_cartesian_array_to_spherical_array(triangle_centroid_array)
    )
    array_triangle_centroid_spherical_coords[..., 0] = 1.0
    triangle_centroid_array = convert_spherical_array_to_cartesian_array(
        array_triangle_centroid_spherical_coords
    )
    ...
    triangle_to_normal_distance_array = numpy.linalg.norm(
        triangle_centroid_array - facet_normal_unit_vector_array, axis=1
    )
    sphere_centroid_to_normal_distance_array = numpy.linalg.norm(
        sphere_centroid - facet_normal_unit_vector_array, axis=1
    )
    delta_value_array = (
        sphere_centroid_to_normal_distance_array - triangle_to_normal_distance_array
    )
    facet_normal_unit_vector_array[
        delta_value_array < -0.1
    ] *= -1.0  # need to rotate the vector so that it faces out of the circle
    ...
    assert array_Voronoi_vertices.shape[1] == 3, (
        "The array of Voronoi vertices on the sphere should have shape (N,3)."
    )
