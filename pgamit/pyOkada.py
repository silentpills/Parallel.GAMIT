...
def inv_azimuthal(x, y, lon, lat):
    ...
    i_lon = lon + atand(
        (x.flatten() * np.sin(c))
        / (r * cosd(lat) * np.cos(c) - y.flatten() * sind(lat) * np.sin(c))
    )
...
class EarthquakeTable(object):
    ...
    def __init__(self, cnn, earthquake_id, include_postseismic=True):
        ...
        stns = cnn.query_float(
            'SELECT "NetworkCode", "StationCode", lat, lon, height, auto_x, auto_y, auto_z '
        ...
        strike = (
            [float(eq["strike1"]), float(eq["strike2"])]
            if not math.isnan(eq["strike1"])
            else []
        )
        dip = (
            [float(eq["dip1"]), float(eq["dip2"])]
            if not math.isnan(eq["strike1"])
            else []
        )
        rake = (
            [float(eq["rake1"]), float(eq["rake2"])]
            if not math.isnan(eq["strike1"])
            else []
        )
        ...
        for stn in stns:
            ...
            sp = (
                a * eq["mag"] - np.log10(dist) + b + np.log10(POST_SEISMIC_SCALE_FACTOR)
            )
    ...
    def get_coseismic_displacements(self, stack="ppp"):
        ...
        etms = self.cnn.query_float(
            f'SELECT "NetworkCode", "StationCode", params, relaxation, '
            f"ARRAY_LENGTH(relaxation,1) as len, jump_type FROM etms "
            f"WHERE jump_type in ({CO_SEISMIC_JUMP_DECAY}, {CO_SEISMIC_JUMP}) "
            f"AND metadata like '%{self.earthquake_id}%' and stack = '{stack}'",
            as_dict=True,
        )
    ...
    def __init__(self, cnn, lat, lon, sdate, edate):
        ...
        jumps = cnn.query_float(
            ...
            % (
                sdate.yyyymmdd(),
                edate.yyyymmdd(),
                (sdate.datetime() - timedelta(days=5 * 365)),
                sdate.yyyymmdd(),
            ),
            as_dict=True,
        )
        ...
        for j in jumps:
            ...
            if s > 0:
                ...
                link = (
                    '<a href="https://earthquake.usgs.gov/earthquakes/eventpage/%s" target="_blank">%s</a>'
                    % (j["id"], j["id"])
                )
                if s_score > 0:
...
class Score(object):
    def __init__(
        ...
        self.gx, self.gy = np.meshgrid(
            np.linspace(-xmax, xmax, density), np.linspace(-xmax, xmax, density)
        )

        if len(self.strike):
            self.c_mx, self.c_my, self.c_mask = self.compute_disp_field()
            self.p_mx, self.p_my, self.p_mask = self.compute_disp_field(
                POST_SEISMIC_SCALE_FACTOR
            )
        else:
            # if not strike information, produce a mask using the L1 S-score only
            self.c_mask = np.sqrt(np.square(self.gx) + np.square(self.gy)) < (
                self.dmax * 1000.0
            )
            self.p_mask = np.sqrt(np.square(self.gx) + np.square(self.gy)) < (
                POST_SEISMIC_SCALE_FACTOR * (self.dmax * 1000.0)
            )

            self.c_mx = self.gx / 1000.0
            self.c_my = self.gy / 1000.0
            self.p_mx = self.gx / 1000.0
            self.p_my = self.gy / 1000.0
    ...
    def fault_dims(self):
        # compute fault dimensions from Wells and Coppersmith 1994
        # all lengths and displacements reported in m
        self.along_strike_l = 10.0 ** (-3.22 + 0.69 * self.mag) * 1000  # [m]
        self.downdip_l = 10.0 ** (-1.01 + 0.32 * self.mag) * 1000  # [m]
        self.avg_disp = 10.0 ** (-4.80 + 0.69 * self.mag)  # [m]
        self.rupture_area = 10.0 ** (-3.49 + 0.91 * self.mag)  # [km ** 2]
        self.maximum_disp = 10.0 ** (-5.46 + 0.82 * self.mag)  # [m]

    def compute_disp_field(self, scale_factor=1.0, limit=1e-3):
        ...
        for depth in self.depth:
            ...
            for strike, dip, rake in zip(self.strike, self.dip, self.rake):
                ...
                # create the mask
                U = np.logical_or(
                    (np.sqrt(np.square(n) + np.square(e) + np.square(u)) >= limit), U
                )
            # print(U)
            ...
            try:
                ref_scale.append(
                    np.max(np.sqrt(np.square(self.gx[U]) + np.square(self.gy[U])))
                )
            except ValueError:
    ...
    def score(self, lat, lon):
        ...
        c = np.arccos(
            sind(self.lat) * sind(lat)
            + cosd(self.lat) * cosd(lat) * cosd(lon - self.lon)
        )
        if c != 0:
            k = c / np.sin(c) * 6371
        else:
        ...
        y = k * (
            cosd(self.lat) * sind(lat)
            - sind(self.lat) * cosd(lat) * cosd(lon - self.lon)
        )
    ...
    def save_masks(self, txt_file=None, kmz_file=None, include_postseismic=False):
        ...
        postseismic_contours = (
            find_contours(self.p_mask, 1e-16) if include_postseismic else []
        )
        ...
        epicenter.style.iconstyle.icon.href = (
        ...
        if self.strike:
            ...
            beachball(
                [self.strike[0], self.dip[0], self.rake[0]],
                width=100,
                linewidth=1,
                facecolor="k",
                outfile=img_buffer,
        ...
        def process_contours(contours, lon_grid, lat_grid, color, name):
            ...
            for outer in outer_polys:
                kml_poly = kml.newpolygon(
                    name=name, outerboundaryis=outer.exterior.coords
                )
                kml_poly.style.linestyle.color = color
                kml_poly.style.linestyle.width = 3
                kml_poly.style.polystyle.color = simplekml.Color.changealphaint(
                    0, simplekml.Color.white
                )
        ...
        process_contours(
            coseismic_contours, c_lon, c_lat, simplekml.Color.blue, "Coseismic mask"
        )
        ...
        if txt_file is not None:
            ...
            if include_postseismic:
                np.savetxt(
                    txt_file,
                    np.column_stack(
                        (c_lon, c_lat, self.c_mask.flatten(), self.p_mask.flatten())
                    ),
                )
            else:
                np.savetxt(
                    txt_file, np.column_stack((c_lon, c_lat, self.c_mask.flatten()))
                )
...
def okadakernel(alp, xi, et, q, sd, cd, disl1, disl2, disl3):
    ...
    if abs(cd) > 1e-6:  # if not vertical fault
        ...
        a5[xi != 0] = (
            ...
            * np.arctan(
                (
                    et[xi != 0] * (x[xi != 0] + q[xi != 0] * cd)
                    + x[xi != 0] * (r[xi != 0] + x[xi != 0]) * sd
                )
