...
def path_replace_tags(
    filename, date: Date, NetworkCode="", StationCode="", Marker=0, CountryCode="ARG"
):
    """
    ...
    rnx2 = RinexNameFormat(
        None,
        StationCode=StationCode,
        monument=str_marker[0],
        receiver=str_marker[1],
        country=CountryCode,
        date=date,
        version=2,
    )
    rnx3 = RinexNameFormat(
        None,
        StationCode=StationCode,
        monument=str_marker[0],
        receiver=str_marker[1],
    ...
    return (
        filename.replace("${year}", str(date.year))
...
class RinexNameFormat(object):
    def __init__(
        ...
        if filename:
            ...
            if version > 2:
                parts = [
                    StationCode + monument + receiver + country,
                    data_source,
                    start_time,
                    file_period,
                    data_frequency,
        ...
        try:
            ...
            raise Exception(
                "Date expection: " + str(e) + " when processing file " + filename
            )
    ...
    def identify_rinex_type(self, filename):
        ...
        ft = next(
            (t for t, ext in version_2_ext.items() if filename.endswith(ext)), None
        )
        ...
        if ft is not None:
            ...
            sfile = re.findall(
                r"[A-Z0-9]{9}_[RSU]_[0-9]{11}_[0-9]{2}[MHDYU]_[0-9]{2}[CZSMHDU]_"
                r"[GREJCISM][OMN]\.(crx|rnx)(.gz|.zip|.bzip2|.bz2)?$",
                filename,
            )
            if not sfile:
                raise RinexNameException(
                    "Could not determine the rinex type (malformed filename): "
                    + filename
                )
            elif sfile[0][0] == "rnx":
                return TYPE_RINEX if sfile[0][1] == "" else TYPE_RINEZ
            elif sfile[0][0] == "crx":
                return TYPE_CRINEX if sfile[0][1] == "" else TYPE_CRINEZ
    ...
    def filename_base(self, override_freq=None):
        if self.version < 3:
            ...
            return (
                ...
                + self.file_period
                + "_"
                + (self.data_frequency if not override_freq else override_freq)
                + "_"
                + self.data_type
            )
    ...
    def split_filename(self, filename):
        if self.version < 3:
            sfile = re.findall(
                r"(\w{4})(\d{3})(\w*)\.(\d{2})([doOD](?:.gz|.Z)?)$", filename
            )

            if sfile:
                if len(sfile[0][2]) > 1:
                    warnings.warn(
                        "Malformed RINEX 2 name: session name is longer than 1 character!"
                    )
                return sfile[0]
            else:
                raise RinexNameException(
                    "Invalid filename format: %s for rinex version %s"
